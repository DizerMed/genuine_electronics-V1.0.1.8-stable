 ...
 */
export function analyzeLoanTransaction(
  tx: POSTransaction,
): OverdueLoanItem | null {
  ...
  const isOverdue = targetDate
    ...
    : meta.isOverdue;
  const overdueDurationText = targetDate
    ? getOverdueDurationText(targetDate)
    : "No time limit set";
 ...
 */
export function scanOverdueLoansList(
