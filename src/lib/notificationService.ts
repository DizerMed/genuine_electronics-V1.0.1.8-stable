...
export function buildNotificationMessage(
  ...
  if (type === "ORDER_CONFIRMATION" && order) {
    const totalFormatted = formatTZS(
      order.totalAmount || order.total_amount || 0,
    );
    const itemsSummary = (order.items || [])
      .map(
        (it) =>
          `• ${it.quantity}x ${it.product?.name || "Item"} (${formatTZS((it.price || it.product?.price || 0) * it.quantity)})`,
      )
      .join("\n");

    return (
      `🛍️ *${storeName} - UTHIBITISHO WA ODA*\n\n` +
  ...
  if (type === "DISPATCH_UPDATE" && order) {
    const courier =
      order.courierName || order.courier || "Genuine Express Dispatch";
    const tracking =
      order.trackingNumber ||
      order.tracking_number ||
      "GE-EXP-" + order.id.slice(-6).toUpperCase();
    const eta = order.estimatedDelivery || "Leo (Masaa 2 - 4)";

    return (
      `🚚 *${storeName} - ODA YAKO IMESAFIRISHWA!*\n\n` +
  ...
  if (type === "LOAN_REMINDER" && (order || posTransaction)) {
    ...
    const paid =
      tx.paidAmount || tx.paid_amount || tx.downPayment || tx.down_payment || 0;
    const deadline =
      tx.loanDueDate ||
      tx.loan_due_date ||
      tx.deadline ||
    ...
    return (
      `⚠️ *${storeName} - KUMBUKUMBU YA MAREJESHO YA MKOPO*\n\n` +
  ...
  if (type === "RECEIPT_SHARE" && posTransaction) {
    ...
    const itemsText = (posTransaction.items || [])
      .map(
        (it) =>
          `• ${it.quantity}x ${it.product?.name || "Item"} @ ${formatTZS(it.price || it.product?.price || 0)}`,
      )
      .join("\n");

    return (
      `🧾 *${storeName} - RISITI YA KIELEKTRONIKI*\n\n` +
      `🏪 *Duka:* ${storeAddress}\n` +
      ...
      (posTransaction.includeVat
        ? `🏛️ *TRA VAT (18%):* Imelipwa (Kodi Imejumuishwa)\n`
        : "") +
