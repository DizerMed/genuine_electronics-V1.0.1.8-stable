...
export async function recordAuditLog(
  ...
  try {
    ...
    const actorName =
      params.actorName ||
      (actorEmail.includes("admin") ? "Super Admin" : actorEmail.split("@")[0]);
    const actorRole =
      params.actorRole ||
      (actorEmail.includes("admin") ? "Super Admin" : "Staff");
...
export function exportAuditLogsToCSV(logs: AuditLog[]): void {
  ...
  link.setAttribute(
    "download",
    `Audit_Logs_Genuine_Electronics_${new Date().toISOString().split("T")[0]}.csv`,
  );
