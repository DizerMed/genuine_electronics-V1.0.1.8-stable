...
const InstallPwaBanner = lazy(() =>
  import("./components/InstallPwaBanner").then((m) => ({
    default: m.InstallPwaBanner,
  })),
);
const Footer = lazy(() =>
...
const ClientProfileModal = lazy(() =>
  import("./components/ClientProfileModal").then((m) => ({
    default: m.ClientProfileModal,
  })),
);
...
export default function App() {
  ...
  const [clientCloudOpDetails, setClientCloudOpDetails] = useState<{
    tableName?: string;
    action?: string;
  } | null>(null);
  ...
  useEffect(() => {
    const handleNav = (e: Event) => {
      ...
      if (
        customEvent.detail === "orders" ||
        customEvent.detail === "tracking"
      ) {
        if (!user) {
  ...
  const [clientThemeMode, setClientThemeMode] = useState<
    "system" | "dark" | "light"
  >(() => {
    if (typeof window !== "undefined") {
      try {
  ...
  const [storeSettings, setStoreSettings] = useState<StoreSettings>({
    ...
    announcementText:
      "🎉 Special Offer: Free Express Delivery across Dar es Salaam on orders over TZS 500,000!",
    showAnnouncement: true,
    heroBadge: "Authorized Dealer • 100% Genuine Guarantee",
    heroTitle: "Next-Gen Technology & Home Appliances in Tanzania",
    heroSubtitle:
      "Shop top global brands with official local warranty, official receipts, and same-day delivery.",
    heroImage:
    ...
    paymentMethods: [
      ...
      },
      {
        ...
        instructions: "Send money to this till number.",
        isActive: true,
      },
      {
        id: "3",
      ...
      },
      {
        ...
        accountName: "Cash",
        accountNumber: "N/A",
        instructions: "Pay cash to delivery personnel.",
        isActive: true,
      },
      {
        id: "7",
        ...
        instructions: "Instant Escrow Gateway from Orbi Fintech",
        isActive: false,
      },
    ],
  });
  ...
  const handleUpdateStoreSettings = async (newSettings: StoreSettings) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error("Server returned non-ok for settings sync");
    } catch (err) {
  ...
  useEffect(() => {
    const handleSessionExpired = (e: Event) => {
      ...
      const message =
        detail?.message ||
        "Your session has expired while you were offline. Please sign in again to continue.";
  ...
  const handleAdminLogout = async () => {
    try {
      ...
      console.warn(
        "Backend logout failed, proceeding with local view change",
        err,
      );
    }
    try {
  ...
  const handleAuthSuccess = () => {
    ...
    try {
      ...
      if (session) {
        ...
        if (
          parsed?.email?.toLowerCase() === "admin@genuine-electronics.com" ||
          parsed?.role === "admin"
        ) {
          setCurrentView("admin");
        }
  ...
  const cycleAdminTheme = () => {
    setAdminThemeMode((prev) => {
      ...
      try {
        safeLocalStorage.setItem("ge_admin_theme", next);
      } catch (_) {}
      return next;
    });
  };
  ...
  const cycleClientTheme = () => {
    setClientThemeMode((prev) => {
      const next =
        prev === "system" ? "dark" : prev === "dark" ? "light" : "system";
      try {
        safeLocalStorage.setItem("ge_client_theme", next);
      } catch (_) {}
      return next;
    });
  };
  ...
  useEffect(() => {
    if (currentView === "admin") {
      if (effectiveAdminTheme === "dark")
        document.documentElement.classList.add("dark");
      else document.documentElement.classList.remove("dark");
    } else {
      if (effectiveClientTheme === "dark")
        document.documentElement.classList.add("dark");
      else document.documentElement.classList.remove("dark");
    }
  }, [
  ...
  const {
    data: products,
    addItem: addSupabaseProduct,
    updateItem: updateSupabaseProduct,
    deleteItem: deleteSupabaseProduct,
    clearCollection: clearProducts,
  } = useSupabaseCollection<Product>("products", [], isAdmin);
  const {
    data: orders,
    addItem: addSupabaseOrder,
    updateItem: updateSupabaseOrder,
    deleteItem: deleteOrder,
    clearCollection: clearOrders,
  } = useSupabaseCollection<Order>("orders", [], isAdmin);
  const {
    data: posTransactions,
    addItem: addSupabasePOSTransaction,
    updateItem: updateSupabasePOSTransaction,
    deleteItem: deletePOSTransaction,
    clearCollection: clearPOSTransactions,
  } = useSupabaseCollection<POSTransaction>("posTransactions", [], isAdmin);
  const {
    data: staff,
    updateItem: updateSupabaseStaff,
    deleteItem: deleteSupabaseStaff,
    clearCollection: clearStaff,
  } = useSupabaseCollection<Staff>("staff", [], isAdmin);
  const {
    data: profiles,
    updateItem: updateSupabaseProfile,
    deleteItem: deleteSupabaseProfile,
    clearCollection: clearProfiles,
  } = useSupabaseCollection<UserProfile>("profiles", [], isAdmin);
  const {
    data: categories,
    addItem: addFirestoreCategory,
    updateItem: updateFirestoreCategory,
    deleteItem: deleteFirestoreCategory,
    clearCollection: clearCategories,
  } = useSupabaseCollection<CategoryItem>("categories", [], isAdmin);
  ...
  const addToCart = (product: Product, quantity = 1) => {
    ...
    if (availableStock <= 0) {
      alert(
        `Cannot add "${product.name}" to cart: Item is out of stock (0 available).`,
      );
      return;
    }
    setCart((prev) => {
  ...
  // Order Actions
  const createOrder = (
    orderData: Omit<Order, "id" | "createdAt" | "trackingNumber">,
  ) => {
    ...
    const isCreditCardApp =
      pmApp.includes("credit card") ||
      pmApp.includes("card") ||
      pmApp.includes("visa") ||
      pmApp.includes("mastercard");

    const isLoan = Boolean(
      orderData.isLoan === true ||
      orderData.is_loan === true ||
      (orderData.isLoan !== false &&
        orderData.is_loan !== false &&
        !isCreditCardApp &&
        (pmApp.includes("loan") ||
      ...
      (orderData.isLoan !== false &&
        ...
        Number(orderData.loanBalance) > 0) ||
      (orderData.isLoan !== false &&
        orderData.is_loan !== false &&
        Boolean(orderData.loanDueDate || orderData.loan_due_date)),
    );
    ...
    const downPayment = isLoan
      ? Number(orderData.downPayment ?? orderData.down_payment ?? paid)
      : 0;
    ...
    const loanDueDateVal = isLoan
      ? orderData.loanDueDate ||
        orderData.loan_due_date ||
        orderData.loanDueDateTime ||
    ...
    let loanStatus: "unpaid" | "partial" | "paid" | "overdue" | undefined =
      isLoan
        ? orderData.loanStatus || (orderData.loan_status as any)
        : undefined;
    ...
    const newOrder: Order = {
      ...
      loanDueTime: isLoan
        ? orderData.loanDueTime || orderData.loan_due_time
        : undefined,
      loan_due_time: isLoan
        ? orderData.loanDueTime || orderData.loan_due_time
        : undefined,
      loanDueDateTime: isLoan
        ? orderData.loanDueDateTime ||
          orderData.loan_due_date_time ||
          loanDueDateVal
        : undefined,
      loan_due_date_time: isLoan
        ? orderData.loanDueDateTime ||
          orderData.loan_due_date_time ||
          loanDueDateVal
        : undefined,
      ...
      loanNationalId: isLoan
        ? orderData.loanNationalId || orderData.loan_national_id
        : undefined,
      loan_national_id: isLoan
        ? orderData.loanNationalId || orderData.loan_national_id
        : undefined,
      loanGuarantorName: isLoan
        ? orderData.loanGuarantorName || orderData.loan_guarantor_name
        : undefined,
      loan_guarantor_name: isLoan
        ? orderData.loanGuarantorName || orderData.loan_guarantor_name
        : undefined,
      loanGuarantorPhone: isLoan
        ? orderData.loanGuarantorPhone || orderData.loan_guarantor_phone
        : undefined,
      loan_guarantor_phone: isLoan
        ? orderData.loanGuarantorPhone || orderData.loan_guarantor_phone
        : undefined,
      loanRepayments: isLoan
        ? orderData.loanRepayments ||
          orderData.loan_repayments ||
          orderData.partialPayments ||
          []
        : undefined,
      loan_repayments: isLoan
        ? orderData.loanRepayments ||
          orderData.loan_repayments ||
          orderData.partialPayments ||
          []
  ...
  const addPOSTransaction = async (txData: POSTransaction) => {
    ...
    if (!res || !res.success) {
      throw new Error(
        res?.error || "Failed to save POS transaction to Supabase",
      );
    }
  };
  ...
  const updatePOSTransaction = async (txData: POSTransaction) => {
    ...
    if (!res || !res.success) {
      throw new Error(
        res?.error || "Failed to update POS transaction in Supabase",
      );
    }
  };
  ...
  const addStaff = async (
    staffData: Omit<Staff, "id" | "createdAt"> & { password?: string },
  ) => {
    try {
      ...
      alert(
        `Failed to add staff: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
      throw err;
    }
  };
  ...
  const updateStaff = async (staffData: Staff) => {
    try {
      ...
      alert(
        `Failed to update staff: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
      throw err;
    }
  };
  ...
  const resetCustomerPassword = async (
    ...
    try {
      const response = await fetch(
        `/api/admin/users/${encodeURIComponent(customerId)}/reset-password`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const deleteStaff = async (staffId: string) => {
    try {
      ...
      alert(
        `Failed to delete staff: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    }
  };
  ...
  const deleteCustomer = async (customerId: string, email?: string) => {
    try {
      ...
      // 2. Authoritative deletion on backend (Auth + DB + in-memory store)
      const response = await fetch(
        `/api/admin/users/${encodeURIComponent(customerId)}`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
  ...
  const activeTheme =
    currentView === "client" ? effectiveClientTheme : effectiveAdminTheme;
  const isAdminActive =
    currentView === "admin" &&
    Boolean(
      user &&
      (profile?.role === "admin" ||
        user?.email === "admin@genuine-electronics.com"),
    );

  return (
    <div
      className={`${isAdminActive ? "h-screen overflow-hidden" : "min-h-screen"} flex flex-col font-sans transition-colors duration-200 ${
        activeTheme === "dark"
    ...
    >
      {loading ? (
        <div
          className={`min-h-screen flex items-center justify-center w-full ${activeTheme === "dark" ? "bg-[#020617] text-white" : "bg-slate-50 text-slate-900"}`}
        >
          <div className="flex flex-col items-center gap-4">
            <div className="w-12 h-12 border-4 border-blue-600/30 border-t-blue-600 rounded-full animate-spin"></div>
            <p
              className={`text-sm font-medium ${activeTheme === "dark" ? "text-slate-400" : "text-slate-600"}`}
            >
              Securing Connection...
            </p>
            <p
              className={`text-xs ${activeTheme === "dark" ? "text-slate-500" : "text-slate-400"}`}
            >
              {authLoading
        ...
        <>
          ...
          {currentView === "client" ? (
                ...
                <Navbar
                  ...
                  onOpenWishlist={() => {
                    window.dispatchEvent(
                      new CustomEvent("nav-action", { detail: "wishlist" }),
                    );
                  }}
                  onOpenOrders={() => {
                    setProfileModalTab("orders");
            ...
            user?.email === "admin@genuine-electronics.com" ? (
              ...
              <div
                className={`min-h-screen flex items-center justify-center p-4 ${effectiveAdminTheme === "dark" ? "bg-slate-950 text-white" : "bg-slate-50 text-slate-900"}`}
              >
                <div
                  className={`max-w-md w-full p-8 rounded-2xl border text-center ${effectiveAdminTheme === "dark" ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200 shadow-xl"}`}
                >
                  <div
                    className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${!profile ? "bg-amber-100 text-amber-600" : "bg-red-100 text-red-600"}`}
                  >
                  ...
                  <p
                    className={`mb-6 ${effectiveAdminTheme === "dark" ? "text-slate-400" : "text-slate-500"}`}
                  >
                    {!profile
          ...
          {currentView === "client" && <CookieConsentBanner />}
