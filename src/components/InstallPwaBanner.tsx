...
export const InstallPwaBanner: React.FC<InstallPwaBannerProps> = ({
  ...
  const [updateStatus, setUpdateStatus] = useState<
    "idle" | "checking" | "available" | "updated"
  >("idle");
  ...
  useEffect(() => {
    ...
    if (checkStandalone) {
      ...
      const isDismissedRecently =
        dismissedTime && Date.now() < Number(dismissedTime);
  ...
  const handleInstallClick = async () => {
    if (deferredPrompt) {
      try {
        ...
        console.warn(
          "Native install prompt failed, switching to manual guide UI:",
          err,
        );
        setShowManualModal(true);
      }
  ...
  const handleResetPwa = async () => {
    ...
    if (typeof window !== "undefined" && "serviceWorker" in navigator) {
      try {
        ...
        console.warn(
          "Failed to unregister SW or clear caches during PWA reset:",
          err,
        );
      }
    }
    ...
    <>
        ...
        >
          <div
            className={`relative overflow-hidden rounded-2xl shadow-2xl border p-3.5 sm:p-4 transition-all ${
              isDark
                ? "bg-slate-900 border-slate-700/80 shadow-black/50 text-white"
                : "bg-white border-slate-200 shadow-slate-200/50 text-slate-900"
            }`}
          >
            <div
              className={`absolute top-0 left-0 right-0 h-1 ${isDark ? "bg-slate-800" : "bg-slate-100"}`}
            >
              <div
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                >
                  {isSw
                    ? "Weka App / Shortcut kwa ununuzi wa haraka bila mtandao"
                    : "Install App / Shortcut for fast & offline shopping"}
                </p>
                ...
                {updateStatus === "checking" && (
                  <div className="mt-2 flex items-center gap-1.5 text-[11px] font-semibold text-blue-600 dark:text-blue-400 animate-pulse">
                    ...
                    <span>
                      {isSw ? "Inakagua sasisho..." : "Checking for updates..."}
                    </span>
                  </div>
                )}
                {updateStatus === "available" && (
                  <div className="mt-2 flex items-center gap-1.5 text-[11px] font-bold text-amber-600 dark:text-amber-400">
                    ...
                    <button
                      ...
                      className="ml-1 underline font-black hover:text-amber-700 dark:hover:text-amber-300"
                    >
                      {isSw ? "Sasisha" : "Reload"}
                    </button>
                  </div>
                ...
                {updateStatus === "updated" && (
                  <div className="mt-2 flex items-center gap-1.5 text-[11px] font-bold text-emerald-600 dark:text-emerald-400">
                    ...
                    <span>
                      {isSw ? "Toleo la hivi karibuni" : "App up to date"}
                    </span>
                  </div>
                )}
              </div>
      ...
      {showManualModal && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          ...
          >
            <div
              className={`p-5 sm:p-6 border-b flex items-start justify-between gap-4 ${isDark ? "border-slate-800" : "border-slate-100"}`}
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black tracking-tight">
                    {isSw
                      ? "Weka Genuine App / Shortcut"
                      : "Install Genuine App / Shortcut"}
                  </h3>
                  <p
                    className={`text-xs mt-0.5 ${isDark ? "text-slate-400" : "text-slate-500"}`}
                  >
                    {isSw
                      ? "Weka Genuine Electronics kwenye Home Screen ya simu au kompyuta yako"
                      : "Add Genuine Electronics icon directly to your phone or PC Home Screen"}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-5 sm:p-6 space-y-5">
                  ...
                  <div className="min-w-0">
                    <p className="font-bold truncate">
                      {updateStatus === "checking"
                        ...
                        : updateStatus === "available"
                          ? isSw
                            ? "Sasisho jipya la PWA linapatikana!"
                          ...
                          : updateStatus === "updated"
                            ...
                            : isSw
                              ? "Toleo jipya limewashwa (PWA active)"
                              : "Latest version active (PWA online)"}
                    </p>
                    {lastCheckTime && (
                      <p
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {isSw
                      ? "Kama ulifuta App (uninstalled), unaweza kuipakua tena hapa."
                      : "If you previously uninstalled the App, you can re-install it here."}
                  </span>
                </div>
                <button
              ...
              {isInstalled ? (
                ...
                >
                  ...
                  <h4
                    className={`text-sm font-black mb-1.5 ${isDark ? "text-emerald-400" : "text-emerald-700"}`}
                  >
                ...
                <>
                  {/* Tabs */}
                  <div
                    className={`flex items-center p-1 rounded-2xl text-xs font-bold ${isDark ? "bg-slate-800/50" : "bg-slate-100"}`}
                  >
                    <button
                      ...
                      <>
                        ...
                        >
                          ...
                          <div className="text-xs">
                            ...
                            >
                              {isSw
                                ? "Fungua kwenye Safari Browser"
                                : "Open in Safari Browser"}
                            </p>
                            <p
                              className={`mt-0.5 ${isDark ? "text-slate-400" : "text-slate-500"}`}
                            >
                              {isSw
                                ? "Kwenye iPhone/iPad, tumia kivinjari cha Safari. Kama unatumia Chrome, nakili Link ya App kisha ifungue kwenye Safari."
                                : "On iPhone/iPad, use Safari browser. If using Chrome, copy the link and open it in Safari."}
                            </p>
                            <button
                        ...
                        >
                          ...
                          <div className="text-xs">
                            <p
                              className={`font-bold ${isDark ? "text-slate-100" : "text-slate-900"}`}
                            >
                              {isSw ? "Bofya Share Icon" : "Tap Share Icon"}
                            </p>
                            <p
                              className={`mt-0.5 flex items-center gap-1.5 ${isDark ? "text-slate-400" : "text-slate-500"}`}
                            >
                              {isSw ? "Bofya alama ya" : "Tap the"}{" "}
                              <Share
                                className={`w-3.5 h-3.5 inline ${isDark ? "text-blue-400" : "text-blue-600"}`}
                              />{" "}
                              {isSw
                                ? "iliyopo chini ya kivinjari cha Safari."
                      ...
                      <>
                        ...
                        >
                          ...
                          <div className="text-xs">
                            ...
                            >
                              {isSw
                                ? "Fungua Menu ya Chrome / Browser"
                                : "Open Browser Menu"}
                            </p>
                            <p
                              className={`mt-0.5 flex items-center gap-1.5 ${isDark ? "text-slate-400" : "text-slate-500"}`}
                            >
                              {isSw ? "Bofya nukta tatu" : "Tap three dots"}{" "}
                              <MoreVertical
                                className={`w-3.5 h-3.5 inline ${isDark ? "text-blue-400" : "text-blue-600"}`}
                              />{" "}
                              {isSw
                                ? "juu kulia ya kivinjari chako cha Chrome."
                                : "in top right corner of Chrome."}
                            </p>
                          </div>
                        </div>
                        ...
                        >
                          ...
                          <div className="text-xs">
                            ...
                            >
                              {isSw
                                ? 'Chagua "Install App" au "Add to Home screen"'
                      ...
                      <>
                        ...
                        >
                          ...
                          <div className="text-xs">
                            ...
                            >
                              {isSw
                                ? "Bofya Alama ya Download kwenye Address Bar"
                                : "Click Address Bar Download Icon"}
                            </p>
                            <p
                              className={`mt-0.5 flex items-center gap-1.5 ${isDark ? "text-slate-400" : "text-slate-500"}`}
                            >
                              {isSw
                                ? "Tazama upande wa kulia wa Address bar yenye alama ya"
                                : "Look at the right side of address bar for"}{" "}
                              <Download
                                className={`w-3.5 h-3.5 inline ${isDark ? "text-blue-400" : "text-blue-600"}`}
                              />
                              .
                        ...
                        >
                          ...
                          <div className="text-xs">
                            ...
                            >
                              {isSw
                                ? 'Bofya "Install" ili kuweka App icon kwenye Desktop ya kompyuta yako.'
                                : 'Click "Install" to create desktop app shortcut.'}
                            </p>
                          </div>
                        </div>
                      </>
            ...
            >
              {deferredPrompt ? (
                ...
                >
                  ...
                  <span>
                    {isSw
                      ? "Pakua Moja kwa Moja (Direct Install)"
                      : "Direct Install App"}
                  </span>
                </button>
              ) : (
                ...
                >
                  <span>
                    {isSw ? "Nimeelewa, Ahsante!" : "Got It, Thanks!"}
                  </span>
                </button>
              )}
            </div>
