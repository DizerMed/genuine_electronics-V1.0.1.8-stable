...
export const POSZReportModal: React.FC<POSZReportModalProps> = ({
  ...
  const totalSales = filteredTransactions.reduce(
    (sum, t) => sum + (t.total || t.totalAmount || 0),
    0,
  );
  ...
  const totalNetSubtotal = filteredTransactions.reduce(
  ...
  const cashDiscrepancy =
    actualClosingCash > 0 ? actualClosingCash - expectedClosingCash : 0;
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `Z_REPORT_${selectedDate}_${(storeSettings?.storeName || "GENUINE").replace(/\s+/g, "_")}.csv`,
    );
    ...
    >
      <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden my-8">
        ...
        <div className="flex items-center justify-between p-4 bg-slate-900 text-white border-b border-slate-800 no-print">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-base tracking-wide">
              POS Z-Report (Daily Register Closure)
            </h3>
          </div>
          <div className="flex items-center gap-2">
            <button
        ...
        <div className="p-4 bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-700/50 no-print flex flex-wrap items-center gap-4 text-xs">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              onChange={(e) =>
                setActualClosingCash(Math.max(0, Number(e.target.value) || 0))
              }
        ...
        <div className="p-6 overflow-y-auto max-h-[70vh] bg-white text-black font-mono">
          <div
            ref={reportRef}
            className="max-w-[480px] mx-auto p-4 border-2 border-black space-y-3 text-xs leading-relaxed"
          >
            {/* Store Banner */}
            <div className="text-center pb-2 border-b-2 border-black space-y-0.5">
              ...
              <p className="font-bold text-[11px]">
                END OF DAY REGISTER CLOSURE (Z-REPORT)
              </p>
            ...
            <div className="py-1 border-b border-black text-[11px] font-bold space-y-0.5">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-black">
                  {formatToGMT3(new Date().toISOString())}
                </span>
              </div>
              <div className="flex justify-between">
                <span>CASHIER / OPERATOR:</span>
                <span className="font-black">
                  {cashierName || "Primary Cashier"}
                </span>
              </div>
              <div className="flex justify-between">
            ...
            <div className="py-1 border-b-2 border-black text-[11px] font-bold space-y-1">
              <p className="font-black uppercase tracking-wider text-[11px] border-b border-dashed border-black pb-0.5">
                SALES REVENUE SUMMARY
              </p>
              <div className="flex justify-between">
                <span>GROSS SALES:</span>
                <span className="font-black">
                  {formatTZS(totalSales + totalDiscount)}
                </span>
              </div>
              {totalDiscount > 0 && (
              ...
              {Object.keys(paymentBreakdown).length === 0 ? (
                ...
                Object.entries(paymentBreakdown).map(([method, data]) => (
                  <div key={method} className="flex justify-between">
                    <span>
                      {method.toUpperCase()} ({data.count}x):
                    </span>
              ...
              {actualClosingCash > 0 && (
                <>
                  <div className="flex justify-between">
                    ...
                    <span className="font-black">
                      {formatTZS(actualClosingCash)}
                    </span>
                  </div>
                  <div
                    className={`flex justify-between font-black border-t border-black pt-0.5 ${cashDiscrepancy === 0 ? "text-green-800" : cashDiscrepancy < 0 ? "text-red-800" : "text-blue-800"}`}
                  >
                    <span>VARIANCE / DISCREPANCY:</span>
                    <span>
                      {cashDiscrepancy >= 0
                        ? `+${formatTZS(cashDiscrepancy)}`
                        : `-${formatTZS(Math.abs(cashDiscrepancy))}`}
                    </span>
                  </div>
                </>
              )}
