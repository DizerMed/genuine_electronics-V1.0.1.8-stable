...
export const ProductCompareModal: React.FC<ProductCompareModalProps> = ({
  ...
  const allSpecKeys: string[] = Array.from(
    new Set<string>(compareProducts.flatMap((p) => Object.keys(p.specs || {}))),
  );
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/70 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 animate-fadeIn">
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border-none w-full max-w-6xl max-h-[92vh] flex flex-col overflow-hidden my-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Analyze specs, features, and prices side-by-side to find your
                ideal genuine electronic.
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
          ...
          {compareProducts.length === 0 ? (
            <div
              className={`text-center py-16 rounded-2xl border-2 border-dashed p-8 ${
                isDark
            ...
            >
              ...
              />
              <h3
                className={`text-lg font-extrabold ${isDark ? "text-white" : "text-slate-800"}`}
              >
                No products selected for comparison
              </h3>
              <p
                className={`text-xs mt-1 max-w-md mx-auto ${isDark ? "text-slate-400" : "text-slate-500"}`}
              >
                Select up to 3 products while browsing the catalog by clicking
                the <strong>Compare</strong> button on any item.
              </p>
              <button
                onClick={onClose}
            ...
            <>
              ...
              {compareProducts.length < 3 && (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-xs font-bold ${isDark ? "text-blue-300" : "text-blue-900"}`}
                    >
                      You can add {3 - compareProducts.length} more product
                      {3 - compareProducts.length > 1 ? "s" : ""} to compare!
                    </span>
                  </div>
                  ...
                  {isAdding && availableToAdd.length > 0 && (
                    <div
                      className={`w-full sm:col-span-2 border rounded-2xl shadow-xl p-2 max-h-48 overflow-y-auto space-y-1 mt-2 ${
                        isDark
                        ...
                        >
                          <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              >
                                {product.brand} • {formatTZS(product.price)}
                              </p>
                            </div>
                          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-center mb-4">
                        ...
                        <p
                          className={`text-xs font-bold uppercase tracking-wider mb-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}
                        >
                          {product.brand}
                        </p>
                        <h3
                      ...
                      {/* Price & Rating */}
                      <div
                        className={`p-3.5 rounded-xl border mb-4 text-center ${
                          isDark
                      ...
                      <div
                        className={`space-y-2 text-xs mb-4 border-t pt-3 ${isDark ? "border-slate-700/60" : "border-slate-100"}`}
                      >
                        <div
                          className={`flex justify-between py-1 border-b ${isDark ? "border-slate-800" : "border-slate-50"}`}
                        >
                          <span
              ...
              {allSpecKeys.length > 0 && (
                <div
                  className={`rounded-2xl border p-4 sm:p-6 mt-6 ${
                    isDark
                      ? "bg-slate-900 border-slate-800"
                ...
                >
                  <h4
                    className={`text-base font-extrabold mb-4 flex items-center gap-2 ${isDark ? "text-white" : "text-slate-900"}`}
                  >
                  ...
                  >
                    <table className="w-full text-xs text-left border-collapse">
                      <thead>
                        ...
                        >
                          <th
                            className={`p-3 w-1/4 border-r ${isDark ? "border-slate-800 bg-slate-900" : "border-slate-200 bg-slate-100"}`}
                          >
                            Specification
                          </th>
                          {compareProducts.map((p) => (
                            <th
                              key={p.id}
                              className={`p-3 border-r last:border-r-0 min-w-[160px] ${isDark ? "border-slate-800 text-white" : "border-slate-200 text-slate-900"}`}
                            >
                              {p.name}
                            </th>
                          ))}
                      ...
                      >
                        {allSpecKeys.map((key) => {
                          const values = compareProducts.map(
                            (p) => p.specs?.[key] || "N/A",
                          );
                          ...
                          return (
                            ...
                            >
                              <td
                                className={`p-3 font-semibold border-r ${
                                  isDark
        ...
        >
          ...
          >
            ...
            <span>
              All products listed are 100% genuine with official manufacturer
              warranty.
            </span>
          </div>
          <button
