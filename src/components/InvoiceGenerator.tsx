...
export const InvoiceGenerator: React.FC<InvoiceGeneratorProps> = ({
  ...
  className = "",
  defaultIncludeVat,
  defaultDocType = order.paymentStatus === "Paid" ||
  ...
  const [docType, setDocType] = useState<"tax" | "proforma" | "delivery">(
    defaultDocType,
  );
  ...
  const initialVatChecked = (() => {
    ...
    if (order.items && order.items.length > 0) {
      const allItemsExempt = order.items.every(
    ...
    if (order.vatPercentage !== undefined && Number(order.vatPercentage) === 0)
      return false;
    if (order.tax !== undefined && Number(order.tax) === 0) return false;
    if (
      ...
      return false;
    if (order.items && order.items.length > 0) {
      const hasExplicitVatInfo = order.items.some(
        (item) => item.product?.isVatInclusive !== undefined,
      );
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        shareDropdownActionRef.current &&
        !shareDropdownActionRef.current.contains(event.target as Node)
      ) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      alert(
        "Could not generate PDF file directly. Opening system print window instead.",
      );
      window.print();
    } finally {
  ...
  const defaultStoreVatPct = Math.min(
    100,
    Math.max(0, Number(storeSettings?.vatPercentage ?? 18)),
  );
  const baseOrderVatPct =
    order.vatPercentage !== undefined && order.vatPercentage > 0
      ? order.vatPercentage
      : defaultStoreVatPct;
  ...
  const orderExtraCosts =
    order.extraCosts &&
    Array.isArray(order.extraCosts) &&
  ...
  const getInvoiceText = () => {
    const docTitle =
      docType === "tax"
        ? includeVat && activeVatPercentage > 0
    ...
    (order.items || []).forEach((item, idx) => {
      ...
      const vatTag =
        includeVat && item.product?.isVatInclusive !== false ? " [VAT]" : "";
    ...
    if (order.isLoan) {
      ...
      const repaymentsSum = repayments.reduce(
    ...
    if (docType !== "delivery" && selectedMethod) {
      ...
      if (selectedMethod.instructions)
        msg += `Instructions: ${selectedMethod.instructions}\n`;
    }
  ...
  const defaultPaymentMethods = [
    {
      ...
      instructions:
        "Transfer directly to CRDB account. Quote invoice number as reference.",
      isActive: true,
    },
    {
      id: "2",
    ...
    {
      id: "3",
      type: "Orbi Pay" as const,
      provider: "Orbi Pay Wallet",
      accountName: "Genuine Electronics Ltd",
      accountNumber: "ORBI-9901",
      instructions: "Pay via Orbi Pay barcode/wallet tag.",
      isActive: true,
    },
  ];
  ...
  return (
    <div
      className={`invoice-generator-container flex flex-col h-full min-h-0 overflow-hidden ${className}`}
    >
      ...
      {showControls &&
        ...
        order.status !== "Completed" && (
          <div className="bg-amber-500/15 border-b border-amber-500/30 px-4 py-2 text-center text-xs font-semibold text-amber-300 no-print flex items-center justify-center gap-2 shrink-0">
            ...
            <span>
              Payment Confirmation Pending — Displaying Proforma Invoice for
              Order #{order.id}. Official Payment Receipt unlocks once payment
              is confirmed.
            </span>
          </div>
        )}
        ...
        <div className="no-print bg-slate-900 text-white px-4 sm:px-5 py-3.5 flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 rounded-t-2xl shrink-0">
              ...
              <p className="text-[10px] text-slate-400">
                {docType === "tax"
                  ? includeVat && activeVatPercentage > 0 && calculatedTax > 0
                    ? `TRA-Compliant Fiscal Sales Invoice (${activeVatPercentage}% VAT Included)`
                    : "Commercial Sales Invoice"
                  : docType === "proforma"
          ...
          <div className="flex items-center flex-wrap gap-2">
            ...
            >
              ...
              <span>
                Print {docType === "delivery" ? "Delivery Note" : "Invoice"}
              </span>
            </button>
            {onClose && (
              <button
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start pb-4 border-b-2 border-slate-900 gap-4">
              ...
              <div className="text-[11px] text-slate-700 mt-2 space-y-0.5">
                <p className="font-bold text-slate-900">
                  {storeSettings?.storeName ||
                    "Genuine Electronics Tanzania Ltd"}{" "}
                  •{" "}
                  {storeSettings?.address ||
                    "Kariakoo / Ndanda na Masasi Street, Dar es Salaam Tanzania"}
                </p>
                <p>
                  Hotline: {storeSettings?.phone || "+255 768 929 203"} | Email:{" "}
                  {storeSettings?.email || "sales@genuine-electronics.com"}
                </p>
                <p className="font-mono text-[10px] text-slate-600">
                  TRA TIN: {storeSettings?.tin || "104-982-371"}
                </p>
              </div>
            </div>
            ...
            <div className="text-left sm:text-right bg-slate-50 p-3 rounded-xl border border-slate-300 w-full sm:w-auto shrink-0 space-y-1">
              <div
                className={`inline-block px-2.5 py-0.5 rounded border text-[10px] font-black uppercase tracking-wider ${
                  docType === "tax"
                    ? includeVat
              ...
              <div className="text-[11px] text-slate-700 space-y-0.5">
                <p>
                  ...
                  <strong className="text-slate-900">{orderDate}</strong>
                </p>
                <p>
                  <span className="font-semibold text-slate-600">
                    Order ID:
                    ...
                    >
                      <td className="p-2.5">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 text-[9px] text-slate-500 font-mono">
                          <span>
                            SKU: {item.product?.sku || `SKU-${idx + 1}`}
                          </span>
                          {item.product?.brand && (
          ...
          {order.isLoan &&
            (() => {
              ...
              const isOverdue =
                dueDate && dueDate < todayStr && remainingBalance > 0;
              ...
              return (
                <div className="p-3.5 bg-amber-50/90 rounded-2xl border border-amber-200 text-[11px] keep-together space-y-2.5">
                  <div className="flex items-center justify-between font-extrabold text-amber-950">
                    ...
                    <span
                      className={`text-[10px] uppercase px-2 py-0.5 rounded border font-black ${badgeClass}`}
                    >
                      {statusLabel}
                    </span>
                  </div>
                  ...
                  {repayments.length > 0 && (
                    <div className="pt-2 border-t border-amber-200/60 text-[10px] space-y-1">
                      <span className="font-bold text-amber-950 uppercase">
                        Installment Payments History ({repayments.length}):
                      </span>
                        ...
                        {repayments.map((rep, idx) => (
                          ...
                          >
                            <span>
                              •{" "}
                              {rep.date
                                ? formatToGMT3(rep.date).split(",")[0]
                                : "Payment"}{" "}
                              ({rep.paymentMethod || "Cash"})
                            </span>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start gap-4 pt-1 keep-together">
            <div className="text-[11px] text-slate-700 max-w-sm bg-slate-50 p-3 rounded-xl border border-slate-300 space-y-1">
              ...
              <p className="text-[10px] text-slate-600 leading-relaxed">
                All Genuine Electronics products include an official 1-Year
                manufacturer warranty and local Dar es Salaam service support.
                Please retain this physical tax invoice for servicing, warranty
                verifications, and insurance claims.
              </p>
            </div>
            ...
            <div className="w-full sm:w-80 bg-white p-3.5 rounded-xl border-2 border-slate-900 space-y-2 text-[11px]">
              {taxAnalysis.isMixed && taxAnalysis.taxAmount > 0 ? (
                <>
                  ...
                  <div className="flex justify-between text-slate-700">
                    <span className="font-medium">
                      TRA VAT ({activeVatPercentage}% on Taxable):
                    </span>
                    <span className="font-bold text-slate-900">
                ...
                <>
                  ...
                  {includeVat &&
                    ...
                    calculatedTax > 0 && (
                      <div className="flex justify-between text-slate-700">
                        <span className="font-medium">
                          TRA VAT ({activeVatPercentage}% Incl.):
                        </span>
                        <span className="font-bold text-slate-900">
                          {formatTZS(calculatedTax)}
                        </span>
                      </div>
                    )}
                </>
          ...
          <div className="border border-slate-300 rounded-xl p-3.5 bg-slate-50 space-y-2.5 text-[11px] keep-together">
              ...
              <div className="bg-white p-3 rounded-xl border border-slate-300 space-y-1.5">
                <div className="flex items-center justify-between">
                  <div className="text-slate-900 font-extrabold text-xs flex items-center gap-1.5">
                    ...
                    <span>
                      {selectedMethod.provider} ({selectedMethod.type})
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
                  <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                    <span className="text-slate-500 text-[9px] font-bold block uppercase">
                      Account / Till Number:
                    </span>
              ...
              <div className="bg-white p-3 rounded-xl border border-slate-300 space-y-1">
                <div className="flex items-center justify-between">
                  <div className="text-slate-900 font-extrabold text-xs flex items-center gap-1.5">
                    ...
                    <span>
                      {order.paymentMethod || "Selected Payment Method"}
                    </span>
                  </div>
                ...
                <p className="text-slate-600 text-[10px] pt-1">
                  ...
                  . Please quote invoice reference{" "}
                  <strong className="text-slate-900">{invoiceNo}</strong> during
                  deposit.
                </p>
              </div>
            )}
            ...
            <div className="pt-1.5 border-t border-slate-200 flex items-center justify-between text-[10px]">
              <span className="text-slate-700">
                ...
                <strong className="text-slate-900">{invoiceNo}</strong> as
                payment reference for automated reconciliation.
              </span>
                ...
                {showShareFormatModal && (
                  <div className="absolute bottom-full mb-2 right-0 w-48 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-bottom-2">
                    <div className="p-2 flex flex-col gap-1">
                      <div className="text-[10px] font-black text-slate-500 uppercase px-2 mb-1">
                        Select Format
                      </div>
                      <button
                        type="button"
            ...
            <div className="grid grid-cols-2 gap-4 p-3 rounded-xl border border-slate-300 bg-slate-50 text-[10px] keep-together">
              <div className="space-y-4">
                <p className="font-bold uppercase text-slate-800">
                  Dispatched by (Warehouse/Store Cashier):
                </p>
            ...
            <div className="flex items-center gap-3">
              <div className="p-1.5 bg-white rounded-lg border-2 border-slate-900 shadow-sm">
                <QRCodeSVG
                  value={buildReceiptVerificationUrl({
                    orderNo:
                      (order as any).orderNumber || order.id || invoiceNo,
                    receiptNo: invoiceNo,
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-600">
                  Scan QR Code to verify document authenticity & fiscal status
                  online.
                </p>
                <p className="text-[9px] text-slate-500">
                  {docType === "tax"
                    ? includeVat && activeVatPercentage > 0 && calculatedTax > 0
                      ? "TRA Compliant Commercial Electronic Tax Invoice"
                      : "Commercial Electronic Sales Invoice"
                    : docType === "proforma"
            ...
            <div className="text-right flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-bold text-slate-800 text-[10px]">
                  Financial Operations & Logistics Dept.
                </p>
                <p className="text-[9px] text-slate-500">
