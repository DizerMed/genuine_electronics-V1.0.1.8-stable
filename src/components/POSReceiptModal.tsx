...
import {
  getLoanDueDate,
  isLoanTransaction as checkIsLoanTransaction,
...
export const POSReceiptModal: React.FC<POSReceiptModalProps> = ({
  ...
  const [buyerTin, setBuyerTin] = useState(
    receipt.customerTin || (receipt as any).tin || "",
  );
  ...
  const activeCustomerTin =
    buyerTin.trim() || receipt.customerTin || (receipt as any).tin || "";

  const dbVatRate =
    storeSettings?.vatPercentage && Number(storeSettings.vatPercentage) > 0
  ...
  const allItemsExempt =
    hasReceiptItems &&
    receipt.items!.every((item) => item.product?.isVatInclusive === false);

  const isVatApplicable = (() => {
    if (allItemsExempt) return false;
    if (receipt.includeVat !== undefined) return Boolean(receipt.includeVat);
    if (receipt.vatPercentage !== undefined)
      return Number(receipt.vatPercentage) > 0;
    if (receipt.tax !== undefined) return Number(receipt.tax) > 0;
    if (storeSettings?.vatPercentage !== undefined)
      return Number(storeSettings.vatPercentage) > 0;
    return true;
  })();

  const effectiveVatRate =
    receipt.vatPercentage !== undefined && Number(receipt.vatPercentage) > 0
  ...
  const downPayment = Number(
    receipt.downPayment ?? (receipt as any).down_payment ?? 0,
  );
  const loanRepayments =
    receipt.loanRepayments || (receipt as any).loan_repayments || [];
  const repaymentsSum = loanRepayments.reduce(
    (sum, r) => sum + (Number(r.amount) || 0),
    0,
  );
  const totalPaidToDate = Math.min(receiptTotal, downPayment + repaymentsSum);
  const effectiveLoanBalance =
    receipt.loanBalance !== undefined && !loanRepayments.length
      ? Number(receipt.loanBalance)
      : Math.max(0, receiptTotal - totalPaidToDate);

  const isLoanFullyPaid =
    isLoanTransaction &&
    (effectiveLoanBalance <= 0 || receipt.loanStatus === "paid");
  ...
  const isLoanOverdue = Boolean(
    loanDueDate && loanDueDate < todayDateStr && !isLoanFullyPaid,
  );
  ...
  const getReceiptText = () => {
    const lines: string[] = [
      ...
      showLoanBreakdown
        ? `  RISITI YA MKOPO / CREDIT RECEIPT  `
        : `  RISITI YA MAUZO / CASH RECEIPT  `,
      `========================================`,
      ...
      receipt.loanNationalId
        ? `NIDA / KITAMBULISHO: ${receipt.loanNationalId}`
        : "",
      showLoanBreakdown && receipt.loanGuarantorName
        ? `MDHAMINI: ${receipt.loanGuarantorName} ${receipt.loanGuarantorPhone ? `(${receipt.loanGuarantorPhone})` : ""}`
        : "",
      `----------------------------------------`,
      `BIDHAA / ITEMS:`,
      ...
      ...(receipt.extraCosts && receipt.extraCosts.length > 0
        ? [
            ...
            ...receipt.extraCosts.map(
              (c) => `• ${c.name}: +${formatTZS(c.amount)}`,
            ),
          ]
        : []),
      `----------------------------------------`,
      ...
      !showLoanBreakdown && receipt.tenderedAmount && receipt.tenderedAmount > 0
        ? `PESA ILIYOTOLEWA: ${formatTZS(receipt.tenderedAmount)}`
        : "",
      !showLoanBreakdown && receipt.changeAmount && receipt.changeAmount > 0
        ? `CHENJI ILIYORUDISHWA: ${formatTZS(receipt.changeAmount)}`
        : "",
      ...(showLoanBreakdown
        ? [
            `----------------------------------------`,
            ...
            repaymentsSum > 0
              ? `MALIPO YA AWAMU YALIYOLIPWA: +${formatTZS(repaymentsSum)}`
              : "",
            `JUMLA ILIYOLIPWA: ${formatTZS(totalPaidToDate)}`,
            `SALIO LA MKOPO (OUTSTANDING BALANCE): ${formatTZS(effectiveLoanBalance)}`,
            loanDueDate
              ? `TAREHE YA MWISHO (DUE DATE): ${loanDueDate}${isLoanOverdue ? " (OVERDUE)" : ""}`
              : "",
          ].filter(Boolean)
        : []),
      `========================================`,
            ...
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
              try {
                ...
                if (err?.name !== "AbortError") {
                  console.warn(
                    "Share not permitted, falling back to download:",
                    err?.message || err,
                  );
                } else {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      alert(
        "Could not generate PDF receipt file directly. Opening print window instead.",
      );
      window.print();
    } finally {
            ...
            <div className="relative" ref={shareDropdownRef}>
              ...
              {showShareFormatModal && (
                <div className="absolute top-full mt-2 right-0 w-52 bg-white text-slate-900 border border-slate-200 rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 p-1.5 space-y-1">
                  <div className="text-[10px] font-black text-slate-400 uppercase px-2.5 py-1">
                    Select Format
                  </div>
                  <button
                    type="button"
        ...
        {showBuyerEditor && (
          <div className="no-print bg-slate-950 p-3.5 border-b border-slate-800 text-xs animate-in slide-in-from-top-2 duration-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <div>
                <label
                  htmlFor="receipt-buyer-name"
                  className="block text-[10px] text-slate-400 font-bold mb-1"
                >
                  Customer / Buyer Name
                </label>
                <input
              ...
              <div>
                <label
                  htmlFor="receipt-buyer-tin"
                  className="block text-[10px] text-slate-400 font-bold mb-1"
                >
                  Buyer TIN (Optional for B2B/TRA)
                </label>
                <input
          ...
          >
            ...
            <div className="text-center space-y-1 pb-2 border-b-2 border-black">
              ...
              <div className="text-[11px] sm:text-xs font-black uppercase tracking-wider text-black py-0.5">
                {showLoanBreakdown
                  ? isSwahili
                    ? "RISITI YA MAUZO YA MKOPO"
                    : isBilingual
                      ? "RISITI YA MKOPO / CREDIT SALE RECEIPT"
                      : "CREDIT SALE / LOAN RECEIPT"
                  : isSwahili
                    ? "RISITI YA MAUZO"
                    : isBilingual
                      ? "RISITI YA MAUZO / CASH RECEIPT"
            ...
            <div className="space-y-1 py-2 border-b-2 border-dashed border-black text-[10.5px] text-black font-bold">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-black">
                  {formatToGMT3(receipt.createdAt)}
                </span>
              </div>
              <div className="flex justify-between">
                <span>
              ...
              {showLoanBreakdown && receipt.loanGuarantorName && (
                <div className="flex justify-between items-start gap-1">
                  ...
                  <span className="font-black text-right break-words max-w-[200px]">
                    {receipt.loanGuarantorName}{" "}
                    {receipt.loanGuarantorPhone
                      ? `(${receipt.loanGuarantorPhone})`
                      : ""}
                  </span>
                </div>
                  ...
                  >
                    <div className="text-[11px] text-black font-black leading-snug">
                      ...
                      {item.priceTier === "wholesale" && (
                        <span className="ml-1 text-[9px] bg-black text-white px-1 py-0.2 font-black uppercase">
                          WHOLESALE
                        </span>
                      )}
                      {taxAnalysis.isMixed &&
                        (isItemVat ? (
                          <span className="ml-1 text-[8.5px] border border-black px-1 py-0.2 font-black uppercase tracking-tight">
                            VAT
                          </span>
                        ) : (
                          <span className="ml-1 text-[8.5px] bg-black text-white px-1 py-0.2 font-black uppercase tracking-tight">
                            NON-VAT
                          </span>
                        ))}
                    </div>
                    ...
                    <div className="flex justify-between text-[10.5px] text-black font-extrabold pl-2">
                      <span>
                        {item.quantity} x{" "}
                        {formatTZS(item.price || item.product.price)}
                      </span>
                      <span className="font-black">
                        {formatTZS(
            ...
            <div className="py-2 border-b-2 border-dashed border-black space-y-1 text-[10.5px] text-black font-bold">
              {taxAnalysis.isMixed && taxAnalysis.taxAmount > 0 ? (
                <>
                  ...
                  <div className="flex justify-between text-black">
                    <span>
                      {isSwahili
                        ? "1. BIDHAA ZENYE VAT (KABLA YA KODI):"
                        : isBilingual
                          ? "1. BIDHAA ZENYE VAT / TAXABLE (NET):"
                          : "1. TAXABLE SUBTOTAL (EXCL. VAT):"}
                    </span>
                    <span className="font-black">
                  ...
                  <div className="flex justify-between text-black">
                    <span>
                      {isSwahili
                        ? "2. BIDHAA BILA VAT (MSAMAHA WA KODI):"
                        : isBilingual
                          ? "2. BIDHAA BILA VAT / EXEMPT SUBTOTAL:"
                  ...
                  <div className="flex justify-between text-black">
                    <span>
                      {isSwahili
                        ? `KODI YA VAT (${vatPct}% KWENYE BIDHAA HUSIKA):`
                        : isBilingual
                          ? `KODI YA VAT (${vatPct}%) / VAT (ON TAXABLE):`
                          : `VAT (${vatPct}% ON TAXABLE):`}
                    </span>
                    <span className="font-black">
                      {formatTZS(taxAnalysis.taxAmount)}
                    </span>
                  </div>
                  <div className="flex justify-between text-black pt-0.5 border-t border-dotted border-black/50 font-black">
                    <span>
                      {isSwahili
                        ? "JUMLA YA AWALI BILA KODI (NET):"
                        : isBilingual
                          ? "JUMLA BILA KODI / TOTAL NET SUBTOTAL:"
                          : "TOTAL NET SUBTOTAL:"}
                    </span>
                ...
                <>
                  ...
                  <div className="flex justify-between">
                    <span>
                      {isSwahili
                        ? `KODI YA ONGEZEKO LA THAMANI (VAT ${vatPct}%):`
                        : isBilingual
                          ? `KODI YA VAT (${vatPct}%) / VAT:`
                          : `VAT (${vatPct}% INCL):`}
                    </span>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-black">
                    {formatTZS(receiptTotal + discount)}
                  </span>
                </div>
              )}
              ...
              {receipt.extraCosts && receipt.extraCosts.length > 0 && (
                <div className="space-y-0.5 pt-0.5 border-t border-dashed border-black/40">
                  ...
                  {receipt.extraCosts.map((cost, cIdx) => (
                    <div
                      key={cIdx}
                      className="flex justify-between text-[10px] pl-2 text-black font-semibold"
                    >
                      <span>• {cost.name}:</span>
                      <span className="font-bold">
                        +{formatTZS(cost.amount)}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <div className="flex justify-between items-center py-1.5 my-1 border-t-2 border-b-2 border-black text-black px-1">
                <span className="font-black text-xs sm:text-sm">
                  {showLoanBreakdown
                    ? isSwahili
                      ? "THAMANI YA MKOPO (JUMLA):"
                      : isBilingual
                        ? "JUMLA YA MKOPO / TOTAL VALUE:"
                        : "TOTAL LOAN VALUE:"
                    : isSwahili
                      ? "JUMLA KUU (TZS):"
                      : isBilingual
              ...
              {receipt.splitPayments && receipt.splitPayments.length > 0 ? (
                ...
                receipt.tenderedAmount !== undefined &&
                receipt.tenderedAmount > 0 &&
                !showLoanBreakdown && (
                  <div className="flex justify-between pt-0.5">
                    ...
                    <span className="font-black">
                      {formatTZS(receipt.tenderedAmount)}
                    </span>
                  </div>
                )
              )}
              ...
              {receipt.changeAmount !== undefined &&
                ...
                !showLoanBreakdown && (
                  <div className="flex justify-between font-black pt-0.5">
                    ...
                    <span className="font-black">
                      {formatTZS(receipt.changeAmount)}
                    </span>
                  </div>
                )}
                ...
                <div className="space-y-1 pt-1.5 border-t-2 border-black text-[10.5px]">
                  <div className="flex justify-between items-center border-b border-black pb-0.5">
                    <span className="font-black uppercase tracking-wide">
                      {isSwahili
                        ? "MCHANGANUO WA MKOPO:"
                        : isBilingual
                          ? "MCHANGANUO WA MKOPO / LOAN BREAKDOWN:"
                          : "LOAN & CREDIT BREAKDOWN:"}
                    </span>
                    <span
                    ...
                    >
                      {isSwahili
                        ? loanStatusSwahili
                        : isBilingual
                          ? `${loanStatusSwahili} / ${loanStatusText}`
                          : loanStatusText}
                    </span>
                  </div>
                  ...
                  {repaymentsSum > 0 && (
                    <div className="flex justify-between">
                      <span>
                        {isSwahili
                          ? "MALIPO YA AWAMU YALIYOLIPWA:"
                          : isBilingual
                            ? "MALIPO YA AWAMU / REPAYMENTS PAID:"
                            : "TOTAL REPAYMENTS PAID:"}
                      </span>
                      <span className="font-black">
                        +{formatTZS(repaymentsSum)}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between border-t border-dotted border-black/40 pt-0.5">
                    <span>
                      {isSwahili
                        ? "JUMLA ILIYOLIPWA HADI SASA:"
                        : isBilingual
                          ? "JUMLA ILIYOLIPWA / TOTAL PAID:"
                          : "TOTAL PAID TO DATE:"}
                    </span>
                    <span className="font-black">
                      {formatTZS(totalPaidToDate)}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center py-1 my-0.5 border-t border-b-2 border-black bg-black/5 px-1">
                    <span className="font-black text-[11px] sm:text-xs">
                      {isSwahili
                        ? "SALIO LINALODAIWA (MKOPO):"
                        : isBilingual
                          ? "SALIO LA MKOPO / BALANCE DUE:"
                  ...
                  {loanDueDate && (
                    <div className="flex justify-between items-center text-[10px] font-black pt-0.5">
                      ...
                      <span
                        className={`${isLoanOverdue ? "underline uppercase" : ""}`}
                      >
              ...
              <div className="py-2.5 border-b-2 border-dashed border-black flex justify-center">
                <div className="official-stamp border-2 border-dashed border-[#0033a0] text-[#0033a0] bg-blue-50/80 p-2 px-4 text-center rounded-lg rotate-[-1.5deg] shadow-sm select-none">
                  ...
                  <p className="text-[10.5px] font-black uppercase tracking-wider my-0.5">
                    {showLoanBreakdown
                      ? isSwahili
                        ? "MAUZO YA MKOPO (CREDIT)"
                        : isBilingual
                          ? "MKOPO / CREDIT SALE"
                          : "OFFICIAL CREDIT SALE"
                      : isSwahili
                        ? "IMELIPWA KIKAMILIFU"
                        : isBilingual
                          ? "IMELIPWA / PAID"
                  ...
                  <p className="text-[8.5px] font-mono font-black">
                    TAREHE: {formatToGMT3(receipt.createdAt).split(",")[0]} •
                    VERIFIED
                  </p>
                  {showLoanBreakdown && (
                    <p className="text-[8px] font-mono font-black border-t border-[#0033a0]/30 pt-0.5 mt-0.5">
                      SALIO: {formatTZS(effectiveLoanBalance)} • DUE:{" "}
                      {loanDueDate || "In 30 Days"}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-0.5 text-black font-black">
                <p className="text-[10px] font-black uppercase tracking-wide text-black">
                  {isSwahili
                    ...
                    : isBilingual
                      ? "UTHIBITISHO WA RISITI / VERIFICATION"
