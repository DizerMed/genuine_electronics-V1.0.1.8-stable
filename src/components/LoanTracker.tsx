...
export type LoanStatusFilter =
  "all" | "overdue" | "partial" | "unpaid" | "active" | "paid";
export type LoanPriorityPreset =
  "all" | "overdue" | "due_today" | "due_soon" | "high_balance";
...
export const LoanTracker: React.FC<LoanTrackerProps> = ({
  ...
  const [priorityPreset, setPriorityPreset] =
    useState<LoanPriorityPreset>("all");
  ...
  const [showLoanReceiptStamp, setShowLoanReceiptStamp] =
  ...
  const [loanForDateExtension, setLoanForDateExtension] =
    useState<POSTransaction | null>(null);
  const [newDueDate, setNewDueDate] = useState<string>("");

  // Edit Customer & Loan Details Modal State
  const [loanForEditCustomer, setLoanForEditCustomer] =
    useState<POSTransaction | null>(null);
    ...
    const orderList: (POSTransaction & {
      ...
      .map((ord) => {
        const transformed: POSTransaction & {
          ...
          isLoan: Boolean(
            ord.isLoan ||
            (ord.outstandingBalance && ord.outstandingBalance > 0) ||
            ord.paymentStatus === "Partial",
          ),
          downPayment:
            ord.downPayment ??
            (ord.paidAmount !== undefined &&
            (!ord.loanRepayments || !ord.loanRepayments.length)
              ? ord.paidAmount
              : 0),
  ...
  const getLoanMeta = (tx: POSTransaction) => {
    ...
    return {
      ...
      progressPct:
        meta.total > 0
          ? Math.min(100, Math.round((meta.totalPaid / meta.total) * 100))
          : 100,
    };
  };
  ...
  const metrics = useMemo(() => {
    ...
    const recoveryRate =
      totalLoanedValue > 0
        ? Math.round((totalDebtCollected / totalLoanedValue) * 100)
        : 0;
  ...
  const filteredLoans = useMemo(() => {
    ...
    const filtered = allLoanTransactions.filter((tx) => {
      ...
      if (sourceFilter === "online" && (tx as any)._origin !== "online")
        return false;
      ...
      if (statusFilter === "overdue") {
        ...
        if (
          ...
          !(meta.totalPaid > 0 && meta.remainingBalance > 0 && !meta.isOverdue)
        )
          return false;
      } else if (statusFilter === "unpaid") {
        if (meta.totalPaid > 0 || meta.remainingBalance <= 0 || meta.isOverdue)
          return false;
      ...
      if (priorityPreset === "overdue") {
        ...
        if (meta.remainingBalance <= 0 || meta.dueDate !== todayStr)
          return false;
      } else if (priorityPreset === "due_soon") {
        if (meta.remainingBalance <= 0 || !meta.dueDate) return false;
        const diffDays = Math.ceil(
          (new Date(meta.dueDate + "T00:00:00").getTime() -
            todayDate.getTime()) /
            (1000 * 60 * 60 * 24),
        );
      ...
      if (q) {
        ...
        const matchesName = String(custName || "")
          .toLowerCase()
          .includes(q);
        const matchesPhone = String(custPhone || "")
          .toLowerCase()
          .includes(q);
        const matchesId = (tx.id || "").toLowerCase().includes(q);
        const matchesNationalId = (tx.loanNationalId || "")
          .toLowerCase()
          .includes(q);
        const matchesGuarantor =
          (tx.loanGuarantorName || "").toLowerCase().includes(q) ||
          (tx.loanGuarantorPhone || "").toLowerCase().includes(q);
        const matchesProduct = (tx.items || []).some((item) =>
  ...
  const handleSubmitRepayment = async (e: React.FormEvent) => {
    ...
    if (effectiveAmount <= 0) {
      showAlert(
        "Invalid Amount",
        "Please enter a valid repayment amount greater than 0.",
        "warning",
      );
      return;
    }
    ...
    if (effectiveAmount > meta.remainingBalance + 100) {
      showAlert(
        "Amount Too High",
        `Repayment cannot exceed the remaining balance of ${formatTZS(meta.remainingBalance)}.`,
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      if (
        (selectedLoanForRepayment as any)._origin === "online" &&
        onUpdateOrder
      ) {
        const orig =
          (selectedLoanForRepayment as any)._originalOrder ||
          (orders || []).find((o) => o.id === selectedLoanForRepayment.id);
        if (orig) {
        ...
        const { _origin, _originalOrder, ...cleanTx } =
          selectedLoanForRepayment as any;
        const updatedTx: POSTransaction = {
          ...cleanTx,
          loanBalance: newBalance,
          loanStatus: newStatus,
      ...
      const custPhone =
        selectedLoanForRepayment.customerPhone ||
        getLoanCustomerPhone(updatedTx) ||
        "";
      ...
      showAlert(
        "Error",
        err?.message || "Could not record repayment.",
        "error",
      );
    } finally {
  ...
  const handleSendWhatsAppRepaymentReceipt = (
    tx: POSTransaction,
    rep: LoanRepayment,
    currentBal: number,
    ...
    triggerHaptic("success");
    let rawPhone = (overridePhone || getLoanCustomerPhone(tx)).replace(
      /[^0-9+]/g,
      "",
    );
    if (!rawPhone) return;
  ...
  const handleGenerateAndShareReceiptImage = async (
    tx: POSTransaction,
    rep: LoanRepayment,
    currentBal: number,
  ) => {
        ...
        async (blob) => {
          ...
          try {
            if (navigator.clipboard && window.ClipboardItem) {
              ...
              showAlert(
                "Receipt Copied!",
                "Receipt image copied to clipboard! You can paste it directly into WhatsApp chat.",
                "alert",
              );
              shared = true;
            }
          ...
          if (!shared) {
            ...
            showAlert(
              "Receipt Downloaded",
              "Installment receipt PNG image downloaded to device.",
              "alert",
            );
          }
        },
  ...
  const handleSaveExtendedDueDate = async () => {
    ...
    try {
      ...
      if ((loanForDateExtension as any)._origin === "online" && onUpdateOrder) {
        const orig =
          (loanForDateExtension as any)._originalOrder ||
          (orders || []).find((o) => o.id === loanForDateExtension.id);
        if (orig) {
          const updatedOrder: Order = {
            ...orig,
            loanDueDate: newDueDate,
      ...
      showAlert(
        "Due Date Updated",
        `New payment deadline set to ${newDueDate}.`,
        "alert",
      );
  ...
  const handleOpenEditCustomer = (tx: POSTransaction) => {
    ...
    setEditCustomerName(
      rawName === "Walk-in Customer" ||
        rawName === "Walk-in" ||
        rawName.startsWith("Debtor (") ||
        rawName.startsWith("Customer (")
        ? ""
        : rawName,
    );
    setEditCustomerPhone(getLoanCustomerPhone(tx));
    setEditLoanNationalId(
      tx.loanNationalId ||
        (tx as any).loan_national_id ||
        (tx as any).loannationalid ||
        "",
    );
    setEditLoanDueDate(
      getLoanDueDate(tx) ||
        (tx as any).loanDueDate ||
        (tx as any).loan_due_date ||
        "",
    );
    const guarantor = getLoanGuarantor(tx);
    setEditLoanGuarantorName(
      guarantor.name ||
        tx.loanGuarantorName ||
        (tx as any).loan_guarantor_name ||
        "",
    );
    setEditLoanGuarantorPhone(
      guarantor.phone ||
        tx.loanGuarantorPhone ||
        (tx as any).loan_guarantor_phone ||
        "",
    );
  ...
  const handleSaveCustomerAndLoanInfo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isOnline && onUpdateOrder) {
        const orig =
          (loanForEditCustomer as any)._originalOrder ||
          (orders || []).find((o) => o.id === loanForEditCustomer.id);
        if (orig) {
          const updatedOrder: Order = {
            ...orig,
          ...
          if (
            ...
            typeof (updatedOrder as any).customer === "object"
          ) {
            (updatedOrder as any).customer = {
              ...(updatedOrder as any).customer,
        ...
        if (
          ...
          typeof (updatedTx as any).customer === "object"
        ) {
          (updatedTx as any).customer = {
            ...(updatedTx as any).customer,
            name: cleanCustomerName,
      ...
      showAlert(
        "Account Updated",
        `Customer details and loan deadline for ${editCustomerName.trim() || "account"} have been successfully saved.`,
        "alert",
      );
      ...
      showAlert(
        "Error",
        err?.message || "Could not update loan details.",
        "error",
      );
    } finally {
  ...
  const handleSendWhatsAppReminder = (
    ...
    const guarantorPhone =
      tx.loanGuarantorPhone || (tx as any).loan_guarantor_phone || "";
    const guarantorName =
      tx.loanGuarantorName || (tx as any).loan_guarantor_name || "Mdhamini";

    let rawPhone = (
      overridePhone ||
      (isGuarantorTarget ? guarantorPhone : getLoanCustomerPhone(tx))
    ).replace(/[^0-9+]/g, "");
    if (!rawPhone) {
      const entered = window.prompt(
        isGuarantorTarget
          ? "Enter Guarantor WhatsApp phone number (e.g. 255768...):"
          : "Enter customer WhatsApp phone number (e.g. 255768...):",
        "255",
      );
      if (!entered) return;
    ...
    if (templateType === "urgent") {
      message =
        `🚨 *NOTISI YA HARAKA YA DENI LILILOPITA MUDA (OVERDUE NOTICE)*\n*${storeName.toUpperCase()}*\n----------------------------------------\n` +
        `Ndugu ${custName},\n` +
      ...
      message =
        `🔔 *KUMBUSHO: TAREHE YA MWISHO WA MALIPO NI LEO*\n*${storeName.toUpperCase()}*\n----------------------------------------\n` +
        `Habari ${custName},\n` +
      ...
      message =
        `🤝 *TAARIFA KWA MDHAMINI (GUARANTOR NOTICE)*\n*${storeName.toUpperCase()}*\n----------------------------------------\n` +
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className={`text-xs mt-0.5 ${textSub}`}>
                Track customer installments, outstanding debt balances, due
                dates and collect repayments.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        >
          ...
          <div
            className={`text-[11px] mt-1 flex items-center justify-between font-semibold ${textSub}`}
          >
        ...
        >
          ...
          <div className="text-[11px] mt-1 flex items-center justify-between font-semibold text-emerald-600 dark:text-emerald-400">
            <div className="flex items-center gap-1">
              ...
              <span>
                {metrics.recoveryRate}% recovered ({metrics.paidCount} cleared)
              </span>
            </div>
        ...
        >
          ...
          >
            <span>
              {metrics.activeCount} active balance(s) ({metrics.partialCount}{" "}
              partial)
            </span>
      ...
      <div
        className={`p-4 sm:p-5 rounded-3xl border shadow-sm space-y-4 ${cardBg}`}
      >
        ...
        {(statusFilter !== "all" ||
          ...
          searchQuery) && (
          <div
            className={`p-2.5 rounded-2xl border text-xs flex items-center justify-between gap-2 flex-wrap ${
              isDark
          ...
          >
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-bold flex items-center gap-1">
                ...
                <span>
                  Showing <strong>{filteredLoans.length}</strong> loan
                  {filteredLoans.length === 1 ? "" : "s"} matching:
                </span>
              </span>
              {statusFilter !== "all" && (
              ...
              <span className="text-[11px] font-semibold opacity-80">
                • Filtered Outstanding Debt:{" "}
                <strong>
                  {formatTZS(
                    filteredLoans.reduce(
                      (sum, tx) => sum + getLoanMeta(tx).remainingBalance,
                      0,
                    ),
                  )}
                </strong>
      ...
      {filteredLoans.length === 0 ? (
        <div
          className={`p-12 text-center rounded-3xl border space-y-3 ${cardBg}`}
        >
          ...
          <p className={`text-xs max-w-md mx-auto ${textSub}`}>
            ...
            sourceFilter !== "all"
              ? `No loan transactions matched your current filter criteria (${[
                  ...
                  .filter(Boolean)
                  .join(", ")}). Try adjusting or clearing filters.`
            ...
            return (
              ...
              >
                ...
                <div className="space-y-3">
                  <div className="flex items-start justify-between gap-2 pt-1">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        <h3
                          className={`font-black text-sm truncate ${textTitle}`}
                        >
                  ...
                  {(getLoanCustomerName(tx) === "Walk-in Customer" ||
                    !getLoanCustomerPhone(tx) ||
                    !meta.dueDate) && (
                    <button
                      onClick={() => handleOpenEditCustomer(tx)}
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      {(tx.items || []).slice(0, 2).map((item, idx) => (
                        ...
                        >
                          <span
                            className={`truncate max-w-[170px] ${textTitle}`}
                          >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex justify-between text-[10px] font-bold text-slate-400">
                        <span>
                          Paid: {formatTZS(meta.totalPaid)} ({meta.progressPct}
                          %)
                        </span>
                    ...
                    <div className="flex items-center justify-between text-[11px]">
                      ...
                      {meta.dueDate ? (
                        <div className="flex items-center gap-1.5">
                          <span
                            className={`font-black ${meta.isOverdue && meta.remainingBalance > 0 ? "text-rose-500" : textTitle}`}
                          >
                            {meta.dueDate}
                          </span>
                          <button
                      ...
                      onClick={() => {
                        ...
                        if (!custPhone) {
                          ...
                          setWhatsappReminderModal({
                            isOpen: true,
                            tx,
                            template: meta.isOverdue
                              ? "urgent"
                              : meta.dueDate === todayStr
                                ? "today"
                                : "polite",
                          });
                        }
        ...
        (() => {
          ...
          return (
            <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
              <div
                className={`w-full max-w-lg rounded-3xl border shadow-2xl p-5 sm:p-6 space-y-5 animate-in fade-in zoom-in-95 my-auto ${cardBg}`}
              >
                {/* Header */}
                <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h3 className={`text-base font-bold ${textTitle}`}>
                        Record Loan Installment / Repayment
                      </h3>
                      <p className={`text-[11px] ${textSub}`}>
                        Log partial or full debt payment into customer ledger.
                      </p>
                    </div>
                  </div>
                  <button
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`font-black ${textTitle}`}>
                      {getLoanCustomerName(selectedLoanForRepayment)}{" "}
                      {getLoanCustomerPhone(selectedLoanForRepayment)
                        ? `(${getLoanCustomerPhone(selectedLoanForRepayment)})`
                        : ""}
                    </span>
                  </div>
                ...
                <form onSubmit={handleSubmitRepayment} className="space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className={`text-[10px] ${textSub}`}>
                          Pay via 2 channels (e.g. Cash + M-Pesa)
                        </div>
                      </div>
                    </div>
                    <button
                  ...
                  {!isSplitRepay ? (
                    <>
                      ...
                      <div>
                        <label
                          className={`block text-xs font-bold mb-1.5 ${textSub}`}
                        >
                        ...
                        <input
                          ...
                          value={repayAmount || ""}
                          onChange={(e) =>
                            setRepayAmount(Number(e.target.value) || 0)
                          }
                          required
                        ...
                        <div className="flex items-center gap-1.5 mt-2 flex-wrap text-xs">
                          ...
                          {meta.remainingBalance > 10000 && (
                            <button
                              ...
                              onClick={() =>
                                setRepayAmount(
                                  Math.round(meta.remainingBalance / 2),
                                )
                              }
                            ...
                            >
                              50% (
                              {formatTZS(Math.round(meta.remainingBalance / 2))}
                              )
                            </button>
                          )}
                        </div>
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (val <= meta.remainingBalance) {
                                setSplitAmountB(
                                  Math.max(0, meta.remainingBalance - val),
                                );
                              }
                            }}
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <input
                            ...
                            value={splitAmountB || ""}
                            onChange={(e) =>
                              setSplitAmountB(Number(e.target.value) || 0)
                            }
                            placeholder="Amount TZS"
                            className={`px-3 py-2 rounded-xl text-xs font-bold border outline-none ${inputBg}`}
                          />
                  ...
                  <div className="flex gap-2.5 pt-2">
                    ...
                    <button
                      ...
                      disabled={
                        ...
                        (isSplitRepay
                          ? Number(splitAmountA) + Number(splitAmountB) <= 0
                          : repayAmount <= 0)
                      }
                    ...
                    >
                      ...
                      <span>
                        {isSubmittingRepayment
                          ? "Recording..."
                          : `Confirm & Receive ${formatTZS(isSplitRepay ? Number(splitAmountA) + Number(splitAmountB) : repayAmount)}`}
                      </span>
                    </button>
                  </div>
                </form>
      ...
      {selectedLoanForDetails &&
        (() => {
          ...
          return (
            <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
              <div
                className={`w-full max-w-xl rounded-3xl border shadow-2xl p-5 sm:p-6 space-y-4 animate-in fade-in zoom-in-95 my-auto max-h-[90vh] flex flex-col ${cardBg}`}
              >
                {/* Header */}
                ...
                <div className="space-y-4 overflow-y-auto pr-1 flex-1">
                  ...
                  >
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <span className={`font-black ${textTitle}`}>
                          {getLoanCustomerName(selectedLoanForDetails)}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-400 block text-[10px]">
                        ...
                        <span className={`font-bold ${textTitle}`}>
                          {getLoanCustomerPhone(selectedLoanForDetails) ||
                            "N/A"}
                        </span>
                      </div>
                      <div>
                        ...
                        <span className={`font-bold ${textTitle}`}>
                          {selectedLoanForDetails.loanNationalId ||
                            (selectedLoanForDetails as any).loan_national_id ||
                            (selectedLoanForDetails as any).loannationalid ||
                            "N/A"}
                        </span>
                      </div>
                      <div>
                        ...
                        <span
                          className={`font-black ${meta.isOverdue ? "text-rose-500" : "text-blue-500"}`}
                        >
                          {meta.dueDate ||
                            selectedLoanForDetails.loanDueDate ||
                            (selectedLoanForDetails as any).loan_due_date ||
                            (selectedLoanForDetails as any).loanduedate ||
                            "Open"}
                        </span>
                      </div>
                      {(selectedLoanForDetails.loanGuarantorName ||
                        (selectedLoanForDetails as any)
                          .loan_guarantor_name) && (
                        <div className="col-span-2 pt-1 border-t border-slate-200 dark:border-slate-700">
                          ...
                          <span className={`font-bold ${textTitle}`}>
                            {selectedLoanForDetails.loanGuarantorName ||
                              (selectedLoanForDetails as any)
                                .loan_guarantor_name}{" "}
                            {selectedLoanForDetails.loanGuarantorPhone ||
                            (selectedLoanForDetails as any).loan_guarantor_phone
                              ? `(${selectedLoanForDetails.loanGuarantorPhone || (selectedLoanForDetails as any).loan_guarantor_phone})`
                              : ""}
                          </span>
                        </div>
                      )}
                  ...
                  <div className="space-y-2">
                    <div className="grid grid-cols-3 gap-2 text-center text-xs">
                      <div
                        className={`p-2.5 rounded-xl border ${isDark ? "bg-slate-800/40 border-slate-700" : "bg-slate-100 border-slate-200"}`}
                      >
                        <div className="text-[10px] text-slate-400 font-bold">
                          TOTAL SALE
                        </div>
                        <div
                          className={`font-black text-sm mt-0.5 ${textTitle}`}
                        >
                          {formatTZS(meta.total)}
                        </div>
                      </div>
                    ...
                    <div
                      className={`p-3 rounded-2xl border space-y-1.5 ${isDark ? "bg-slate-800/40 border-slate-700/60" : "bg-slate-50 border-slate-200"}`}
                    >
                      <div className="flex justify-between items-center text-[11px] font-bold">
                        ...
                        >
                          {meta.total > 0
                            ? Math.min(
                                100,
                                Math.round((meta.totalPaid / meta.total) * 100),
                              )
                            : 100}
                          % Paid
                        </span>
                      ...
                      <div className="h-2.5 w-full bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${meta.total > 0 ? Math.min(100, (meta.totalPaid / meta.total) * 100) : 100}%`,
                          }}
                        />
                      </div>
                      <div className="flex justify-between items-center text-[10px] text-slate-400">
                        <span>
                          Deposit:{" "}
                          {formatTZS(selectedLoanForDetails.downPayment || 0)}
                        </span>
                        <span>
                          {meta.remainingBalance <= 0
                            ? "✓ Fully Settled"
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      Items Purchased Under This Loan
                    </h4>
                    <div
                      className={`p-3 rounded-2xl border space-y-1.5 ${isDark ? "bg-slate-800/30 border-slate-700/60" : "bg-slate-50 border-slate-200"}`}
                    >
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <h4
                        className={`text-xs font-bold uppercase tracking-wider ${textSub}`}
                      >
                        Repayments & Installments History
                      </h4>
                      <span className="text-[10px] text-slate-400">
                        {repayments.length} payment(s) logged
                      </span>
                    </div>

                    {repayments.length === 0 ? (
                      <p
                        className={`text-xs text-center py-4 italic ${textSub}`}
                      >
                        No partial repayments logged yet after initial down
                        payment.
                      </p>
                    ) : (
                      <div className="space-y-2">
                          ...
                          >
                            <div>
                              ...
                              <div className="text-[10px] text-slate-400 mt-0.5">
                                {formatToGMT3(rep.date)} · Cashier:{" "}
                                {rep.recordedBy || "Staff"}
                              </div>
                              {rep.notes && (
                                <div
                                  className={`text-[10px] mt-0.5 italic ${textSub}`}
                                >
                                  Ref: {rep.notes}
                                </div>
                              )}
      ...
      {loanForDateExtension && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div
            className={`w-full max-w-sm rounded-3xl border shadow-2xl p-5 space-y-4 animate-in fade-in zoom-in-95 ${cardBg}`}
          >
            <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-800">
              <h3 className={`text-sm font-bold ${textTitle}`}>
                Adjust Loan Payment Deadline
              </h3>
              <button
      ...
      {loanForEditCustomer && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div
            className={`w-full max-w-lg rounded-3xl border shadow-2xl p-5 sm:p-6 space-y-4 animate-in fade-in zoom-in-95 my-auto max-h-[95vh] flex flex-col ${cardBg}`}
          >
            {/* Header */}
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800 shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className={`text-base font-bold ${textTitle}`}>
                    Edit Customer Info & Payment Deadline
                  </h3>
                  <p className={`text-[11px] ${textSub}`}>
                    Update debtor contact details, NIDA identity & loan due
                    date.
                  </p>
                </div>
              </div>
            ...
            {/* Scrollable Form */}
            <form
              onSubmit={handleSaveCustomerAndLoanInfo}
              className="space-y-4 overflow-y-auto pr-1 flex-1"
            >
              {/* Customer Name */}
              <div>
                <label
                  htmlFor="loan-edit-customer-name"
                  className={`block text-xs font-bold mb-1 ${textSub}`}
                >
                  Customer Full Name (Jina la Mteja) *
                </label>
                <input
              ...
              <div>
                <label
                  htmlFor="loan-edit-customer-phone"
                  className={`block text-xs font-bold mb-1 ${textSub}`}
                >
                  Phone Number / WhatsApp (Namba ya Simu) *
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    onBlur={() =>
                      setEditCustomerPhone(formatTzPhone(editCustomerPhone))
                    }
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                  Used for sending WhatsApp balance reminders and SMS
                  statements.
                </p>
              </div>
              ...
              <div
                className={`p-3.5 rounded-2xl border space-y-2.5 ${isDark ? "bg-amber-950/20 border-amber-900/40 text-amber-200" : "bg-amber-50/50 border-amber-200 text-amber-900"}`}
              >
                <div className="flex items-center justify-between">
                  <label
                    htmlFor="loan-edit-due-date"
                    className="text-xs font-bold flex items-center gap-1.5"
                  >
                    <CalendarClock className="w-4 h-4 text-amber-500" />
                    <span>
                      Loan Payment Deadline (Tarehe ya Mwisho wa Malipo) *
                    </span>
                  </label>
                  {editLoanDueDate && (
              ...
              <div>
                <label
                  htmlFor="loan-edit-national-id"
                  className={`block text-xs font-bold mb-1 ${textSub}`}
                >
                  Customer National ID (NIDA) / Identity (Optional)
                </label>
                <div className="relative">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label
                    htmlFor="loan-edit-guarantor-name"
                    className={`block text-xs font-bold mb-1 ${textSub}`}
                  >
                    Guarantor Name (Mdhamini)
                  </label>
                  <input
                ...
                <div>
                  <label
                    htmlFor="loan-edit-guarantor-phone"
                    className={`block text-xs font-bold mb-1 ${textSub}`}
                  >
                    Guarantor Phone
                  </label>
                  <input
                    ...
                    onBlur={() =>
                      setEditLoanGuarantorPhone(
                        formatTzPhone(editLoanGuarantorPhone),
                      )
                    }
      ...
      {whatsappPromptModal.isOpen &&
        ...
        whatsappPromptModal.rep && (
          <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div
              className={`w-full max-w-md rounded-3xl p-6 shadow-2xl border ${isDark ? "bg-slate-900 border-slate-800 text-white" : "bg-white border-slate-200 text-slate-900"}`}
            >
              ...
              <p className={`text-sm font-medium mb-4 ${textSub}`}>
                Installment saved! Would you like to send a payment receipt to
                customer WhatsApp ({whatsappPromptModal.phone || "Enter Phone"})
                now?
              </p>
              ...
              <div className="mb-4 flex items-center justify-between">
                ...
                <div className="pl-3">
                  <label
                    className={`block text-[10px] font-bold mb-1 ${textSub}`}
                  >
                    Official Stamp
                  </label>
                  <button
                    type="button"
                    onClick={() =>
                      setShowLoanReceiptStamp(!showLoanReceiptStamp)
                    }
              ...
              <div className="space-y-2.5">
                ...
                >
                  <span>📸</span>
                  <span>
                    {isGeneratingReceiptImg
                      ? "Generating Receipt Image..."
                      : "Copy / Share Receipt Image (PNG)"}
                  </span>
                </button>
      ...
      {whatsappReminderModal.isOpen &&
        whatsappReminderModal.tx &&
        (() => {
          ...
          return (
            <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
              ...
              >
                ...
                <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className={`text-[11px] ${textSub}`}>
                        Select localized Swahili message template for {custName}
                        .
                      </p>
                    </div>
                  </div>
                ...
                >
                  <div>
                    ...
                    <div className={`text-[11px] ${textSub}`}>
                      Phone: {custPhone || "N/A"}{" "}
                      {guarantorPhone ? `• Mdhamini: ${guarantorPhone}` : ""}
                    </div>
                  </div>
                ...
                {/* Template Options */}
                <div className="space-y-2">
                  <label className={`block text-xs font-bold ${textSub}`}>
                    Select Message Type:
                  </label>
                  <div className="grid grid-cols-1 gap-2">
                    {templates.map((t) => {
                      const isSelected =
                        whatsappReminderModal.template === t.id;
                      return (
                        <button
                          key={t.id}
                          type="button"
                          onClick={() =>
                            setWhatsappReminderModal((prev) => ({
                              ...prev,
                              template: t.id,
                            }))
                          }
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between">
                              <span
                                className={`text-xs font-black ${isSelected ? "text-emerald-500" : textTitle}`}
                              >
                                {t.title}
                              </span>
                              {isSelected && (
                  ...
                  <button
                    ...
                    onClick={() => {
                      handleSendWhatsAppReminder(
                        tx,
                        whatsappReminderModal.template,
                      );
                      setWhatsappReminderModal({
                        isOpen: false,
                        tx: null,
          ...
          whatsappPromptModal.rep && (
            ...
            >
              <div className="text-center pb-2 border-b-2 border-black">
                ...
                <div className="text-xs font-black uppercase tracking-wider py-0.5">
                  RISITI YA MALIPO YA MKOPO (INSTALLMENT RECEIPT)
                </div>
                <p className="text-[10px]">
                  {storeSettings?.address || "Kariakoo, Dar es Salaam"}
                </p>
                <p className="text-[10px]">
              ...
              <div className="py-2 border-b-2 border-dashed border-black space-y-1 text-[11px]">
                <div className="flex justify-between">
                  ...
                  <span className="font-black">
                    {getLoanCustomerName(whatsappPromptModal.tx)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>SIMU:</span>
                  <span className="font-black">
                    {whatsappPromptModal.phone ||
                      getLoanCustomerPhone(whatsappPromptModal.tx)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>AKAUNTI / RISITI:</span>
                  <span className="font-black">
                    {whatsappPromptModal.tx.id}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>TAREHE:</span>
                  <span className="font-black">
                    {formatToGMT3(whatsappPromptModal.rep.date)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>MPOKAJI:</span>
                  <span className="font-black">
                    {whatsappPromptModal.rep.recordedBy ||
                      activeCashierName ||
                      "Staff"}
                  </span>
                </div>
              ...
              <div className="py-2 border-b-2 border-dashed border-black space-y-1 text-[11.5px]">
                ...
                <div className="flex justify-between py-1">
                  ...
                  <span className="font-black">
                    {formatTZS(whatsappPromptModal.rep.amount)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>
                    Njia ya Malipo: {whatsappPromptModal.rep.paymentMethod}
                  </span>
                </div>
              </div>
              <div className="py-2 border-b-2 border-black space-y-1 text-[11.5px]">
                ...
                {whatsappPromptModal.currentBal <= 0 ? (
                  <div className="text-center font-black text-emerald-700 py-1 uppercase">
                    *** DENI LIMEKAMILIKA KULIPWA ***
                  </div>
                ) : (
                  <div className="flex justify-between text-[10.5px]">
                    <span>Tarehe ya Mwisho (Due Date):</span>
                    <span>
                      {getLoanDueDate(whatsappPromptModal.tx) || "N/A"}
                    </span>
                  </div>
                )}
              </div>
              ...
              {showLoanReceiptStamp && (
                <div className="py-2.5 border-b-2 border-dashed border-black flex justify-center">
                  ...
                  >
                    ...
                    >
                      {whatsappPromptModal.currentBal <= 0
                        ? "IMELIPWA KIKAMILIFU • PAID"
                        : "OFFICIAL INSTALLMENT • PAID"}
                    </p>
                    <p
                      style={{
                        fontSize: "8.5px",
                    ...
                    >
                      TAREHE:{" "}
                      {formatToGMT3(whatsappPromptModal.rep.date).split(",")[0]}{" "}
                      • VERIFIED
                    </p>
                  </div>
                </div>
