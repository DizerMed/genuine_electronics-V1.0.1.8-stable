...
export const AdminVisitorAnalytics: React.FC<AdminVisitorAnalyticsProps> = ({
  ...
  const [timeframe, setTimeframe] = useState<
    "today" | "yesterday" | "7days" | "30days" | "60days"
  >("30days");
  const [selectedProductId, setSelectedProductId] = useState<string>("ALL");
  const [selectedInteraction, setSelectedInteraction] = useState<
    VisitorInteractionType | "ALL"
  >("ALL");
  const [selectedDevice, setSelectedDevice] = useState<string>("ALL");
  const [audienceFilter, setAudienceFilter] = useState<
    "customers_only" | "staff_only" | "all"
  >("customers_only");
  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      const data = await fetchVisitorLogs({
        ...
        interactionType:
          selectedInteraction !== "ALL" ? selectedInteraction : undefined,
  ...
  const handlePurgeLogs = async () => {
    if (
      !window.confirm(
        "Are you sure you want to purge visitor logs older than 60 days (2 months)? This will free up database space while preserving recent traffic data.",
      )
    ) {
      return;
    }
  ...
  const handleExportCSV = () => {
    ...
    exportVisitorLogsToCSV(
      logs,
      `genuine_visitor_analytics_${timeframe}_${new Date().toISOString().substring(0, 10)}.csv`,
    );
  };
  ...
  const renderDeviceIcon = (device?: string) => {
    ...
    if (device === "Tablet")
      return <Tablet className="w-3.5 h-3.5 text-indigo-500" />;
    ...
    <div className="space-y-6 pb-12">
      ...
      <div
        className={`rounded-3xl border p-5 sm:p-6 transition-all ${
          isDark
            ? "bg-slate-900/90 border-slate-800 text-slate-100 shadow-lg shadow-black/20"
      ...
      >
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                >
                  Real visitor volume, keyword searches, viewed products, and
                  interaction logs in East Africa Time (EAT).
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            >
              <RotateCw
                className={`w-3.5 h-3.5 ${isLoadingSummary || isLoadingLogs ? "animate-spin text-blue-500" : ""}`}
              />
              <span>Refresh</span>
            </button>
        ...
        >
          <div className="flex items-center gap-2">
            <span
              className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md font-semibold border ${
                isDark
            ...
            <span>
              Logs persist for 60 days maximum and auto-purge to save database
              storage.
            </span>
          </div>
          <div className={isDark ? "text-slate-400" : "text-slate-500"}>
            ...
            <strong className={isDark ? "text-slate-200" : "text-slate-800"}>
              {summary?.retentionInfo?.totalLogsStored || totalLogsCount}
            </strong>{" "}
            records
          </div>
        </div>
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3
                  className={`text-sm sm:text-base font-black ${isDark ? "text-white" : "text-slate-900"}`}
                >
                ...
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${(summary?.activeStaffCount || 0) > 0 ? "bg-emerald-400 animate-pulse" : "bg-slate-400"}`}
                  />
              ...
              >
                Internal staff activity is detected in real-time and isolated so
                customer visitor analytics remain clean and accurate.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 self-start sm:self-center">
            {summary?.activeStaffList && summary.activeStaffList.length > 0 && (
              ...
              >
                ...
                <span>
                  {showStaffList ? "Hide Active Staff" : "View Active Staff"}
                </span>
                <ChevronDown
                  className={`w-3.5 h-3.5 transition-transform ${showStaffList ? "rotate-180" : ""}`}
                />
              </button>
            )}
          </div>
        ...
        {showStaffList &&
          summary?.activeStaffList &&
          summary.activeStaffList.length > 0 && (
              ...
              {summary.activeStaffList.map((st: any) => (
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div
                      className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold text-xs uppercase border ${
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span
                          className={`text-xs font-bold truncate block ${isDark ? "text-white" : "text-slate-900"}`}
                        >
                          {st.name || st.email}
                        </span>
                        <span
                          className={`w-2 h-2 rounded-full shrink-0 ${st.status === "online" ? "bg-emerald-400" : "bg-amber-400"}`}
                          title={st.status}
                        />
                      </div>
                      <span
                        className={`text-[10px] block truncate ${isDark ? "text-slate-400" : "text-slate-500"}`}
                      >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <div className="flex items-center justify-between">
            <span
              className={`text-[10px] font-black uppercase tracking-wider ${isDark ? "text-slate-400" : "text-slate-500"}`}
            >
              Unique Visitors
            </span>
            <div
          ...
          >
            <span>
              ...
              <strong className={isDark ? "text-white" : "text-slate-800"}>
                {summary?.uniqueVisitorsToday ?? 0}
              </strong>
            </span>
            <span>
              Week:{" "}
              <strong className={isDark ? "text-white" : "text-slate-800"}>
                {summary?.uniqueVisitorsWeek ?? 0}
              </strong>
            </span>
          </div>
        </div>
        ...
        >
          <div className="flex items-center justify-between">
            <span
              className={`text-[10px] font-black uppercase tracking-wider ${isDark ? "text-emerald-400" : "text-emerald-600"}`}
            >
              Live Active (15m)
            </span>
            <div
        ...
        >
          <div className="flex items-center justify-between">
            <span
              className={`text-[10px] font-black uppercase tracking-wider ${isDark ? "text-amber-400" : "text-amber-600"}`}
            >
              Products Browsed
            </span>
            <div
        ...
        >
          <div className="flex items-center justify-between">
            <span
              className={`text-[10px] font-black uppercase tracking-wider ${isDark ? "text-purple-400" : "text-purple-600"}`}
            >
              View-to-Cart Rate
            </span>
            <div
          ...
          >
            <span>
              Cart &rarr; Order:{" "}
              <strong className={isDark ? "text-white" : "text-slate-800"}>
                {summary?.cartToOrderRate ?? 0}%
              </strong>
            </span>
            <span>
              Orders:{" "}
              <strong className={isDark ? "text-white" : "text-slate-800"}>
                {summary?.totalOrdersPlaced ?? 0}
              </strong>
            </span>
          </div>
        </div>
        ...
        >
          <div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p
                    className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}
                  >
                    Customer demand terms in search bar
                  </p>
                </div>
              </div>
              <span
            ...
            {summary?.topSearches && summary.topSearches.length > 0 ? (
                ...
                {summary.topSearches.slice(0, 10).map((s, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <span
                        className={`text-xs sm:text-sm font-bold truncate group-hover:text-amber-500 ${
                    ...
                    <div className="flex items-center gap-2.5 text-xs shrink-0">
                      <span
                        className={`font-semibold px-2 py-0.5 rounded-lg border text-[11px] ${
                          isDark
              ...
              <div
                className={`py-12 text-center text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}
              >
          ...
          >
            <span>
              Tip: Click any query to instantly filter logs stream below.
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <h3
            className={`text-sm font-black mb-4 flex items-center gap-2 ${isDark ? "text-white" : "text-slate-900"}`}
          >
          ...
          <div className="space-y-3.5">
            {(summary?.deviceBreakdown || []).map((dev) => (
              <div key={dev.device} className="space-y-1.5">
                <div
                  className={`flex justify-between text-xs font-bold ${isDark ? "text-slate-300" : "text-slate-700"}`}
                >
                ...
                <div
                  className={`w-full h-2 rounded-full overflow-hidden ${isDark ? "bg-slate-800" : "bg-slate-100"}`}
                >
                  <div
        ...
        <div
          className={`lg:col-span-2 rounded-3xl border p-5 transition-all ${
            isDark
              ? "bg-slate-900/90 border-slate-800 text-slate-100 shadow-md"
              : "bg-white border-slate-200 text-slate-900 shadow-xs"
          }`}
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200 dark:border-slate-800">
          <div>
            <h2
              className={`text-base sm:text-lg font-black flex items-center gap-2 ${isDark ? "text-white" : "text-slate-900"}`}
            >
            ...
            <p
              className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}
            >
              Filter real customer events with inline scrollable stream and view
              limits.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          <div>
            <label
              className={`block text-[11px] font-bold uppercase tracking-wider mb-1.5 ${isDark ? "text-slate-300" : "text-slate-700"}`}
            >
              Audience Filter
            </label>
            <select
        ...
        <div className="flex items-center gap-1.5 flex-wrap pt-1">
          <span
            className={`text-[10px] font-black uppercase tracking-wider mr-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}
          >
            Quick Filters:
          </span>
          <button
          ...
          >
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p
                  className={`text-[11px] ${isDark ? "text-blue-400" : "text-blue-700"}`}
                >
                  Category: {activeSelectedProduct.category} • Price: TZS{" "}
                  {(activeSelectedProduct.price || 0).toLocaleString()} • Stock:{" "}
                  {activeSelectedProduct.stock}
                </p>
              </div>
            </div>
            <button
              ...
              className={`text-xs font-bold px-3 py-1.5 rounded-xl border transition-all shrink-0 cursor-pointer ${
                isDark
                  ? "bg-slate-900 border-slate-700 text-blue-300 hover:bg-slate-800"
        ...
        >
          ...
          <div
            className={`p-3.5 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${
              isDark
          ...
          >
            <div className="flex items-center gap-2.5 flex-wrap">
              <span
                className={`text-xs font-black uppercase tracking-wider ${isDark ? "text-slate-300" : "text-slate-800"}`}
              >
                Logs Stream
              </span>
              <span
              ...
              >
                Showing {currentStart}–{currentEnd} of {logs.length} matched
                (Total Stored: {totalLogsCount})
              </span>
            </div>
          ...
          <div className="max-h-[500px] overflow-y-auto overflow-x-auto relative">
            {isLoadingLogs ? (
              <div className="py-24 text-center">
                ...
                >
                  Loading visitor activity stream...
                </p>
              </div>
            ) : logs.length === 0 ? (
              <div className="py-20 text-center space-y-3">
                ...
                <div>
                  ...
                  >
                    No interactions match the selected filter criteria or
                    timeframe.
                  </p>
                </div>
                {hasActiveFilters && (
              ...
              <table className="w-full text-left border-collapse">
                ...
                <thead className="sticky top-0 z-20">
                  <tr
                    className={`text-[11px] font-black uppercase tracking-wider border-b backdrop-blur-md ${
                      isDark
                ...
                <tbody
                  className={`divide-y text-xs ${isDark ? "divide-slate-800/80" : "divide-slate-100"}`}
                >
                  {paginatedLogs.map((log) => {
                    ...
                    return (
                      ...
                      >
                        {/* Timestamp */}
                        <td
                          className={`${rowPadding} whitespace-nowrap font-mono text-[11px] ${
                        ...
                        <td className={rowPadding}>
                          <div
                            className={`font-mono text-[11px] font-bold truncate max-w-[140px] ${
                          ...
                          {log.isAdmin || log.isStaff ? (
                            ...
                            <span
                              className="text-[10px] text-blue-500 font-semibold block truncate max-w-[140px]"
                              title={log.userEmail}
                            >
                        ...
                        <td className={rowPadding}>
                          {log.interactionType === "SEARCH" ? (
                            ...
                            <div className="flex items-center gap-2.5 min-w-0 max-w-[280px]">
                              ...
                              <div className="min-w-0">
                                <span
                                  className={`font-bold truncate block text-xs ${
                                ...
                                <span
                                  className={`text-[10px] ${isDark ? "text-slate-400" : "text-slate-500"}`}
                                >
                                  {log.productCategory || "General"} • TZS{" "}
                                  {(log.productPrice || 0).toLocaleString()}
                                </span>
                              </div>
                            </div>
                          ) : log.categoryFilter ? (
                            ...
                            <span
                              className={`truncate block max-w-[200px] text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}
                              title={log.pageUrl}
                            >
                              {log.pageUrl || "/"}
                            </span>
                        ...
                        <td className={`${rowPadding} whitespace-nowrap`}>
                          <div
                            className={`flex items-center gap-1.5 font-bold ${isDark ? "text-slate-200" : "text-slate-700"}`}
                          >
                            {renderDeviceIcon(log.deviceType)}
                            <span>{log.deviceType || "Mobile"}</span>
                            <span
                              className={`text-[10px] font-normal ${isDark ? "text-slate-500" : "text-slate-400"}`}
                            >
                              ({(log as any).city || "Dar es Salaam"})
                            </span>
                          </div>
                          <span
                            className={`text-[10px] block ${isDark ? "text-slate-500" : "text-slate-400"}`}
                          >
      ...
      {selectedVisitorId && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-xs flex items-center justify-center p-4">
          <div
            className={`rounded-3xl border w-full max-w-2xl max-h-[85vh] shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 ${
              isDark
          ...
          >
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              <p
                className={`text-xs font-semibold ${isDark ? "text-slate-400" : "text-slate-500"}`}
              >
                Step-by-step chronology of this buyer's actions, product views,
                and cart additions:
              </p>

              {visitorJourneyLogs.length === 0 ? (
                <div
                  className={`py-8 text-center text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}
                >
                  No detailed steps found for this visitor in the current cached
                  query.
                </div>
              ) : (
                    ...
                    <div key={step.id} className="relative group">
                      ...
                      >
                        <div className="flex items-center justify-between text-xs">
                          <div className="flex items-center gap-2">
                            <span
                              className={`font-bold ${isDark ? "text-slate-400" : "text-slate-500"}`}
                            >
                              Step {idx + 1}
                            </span>
                        ...
                        {step.searchQuery && (
                          ...
                          >
                            ...
                            ({step.searchResultsCount || 0} hits)
                          </p>
                        )}
