...
export const VisitorActivityHeatmap: React.FC<VisitorActivityHeatmapProps> = ({
  ...
  const [selectedCell, setSelectedCell] =
    useState<VisitorActivityHeatmapCell | null>(null);
  ...
  const recommendedPromo =
    heatmapData?.recommendedPromoWindow ||
    "Friday & Saturday, 8:00 PM - 10:00 PM (EAT)";
  const quietMaintenance =
    heatmapData?.quietMaintenanceWindow || "Daily 02:00 AM – 05:00 AM (EAT)";
  const serverLoad = heatmapData?.serverLoadRating || "OPTIMAL";

  return (
    <div
      className={`rounded-3xl border p-6 transition-all space-y-6 ${isDark ? "bg-slate-900/90 border-slate-800" : "bg-white border-slate-200 shadow-sm"}`}
    >
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              >
                Hourly & day-of-week traffic distribution in East Africa Time
                (EAT, UTC+3) for server load & promo timing.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          {/* Mode Switcher */}
          <div
            className={`p-1 rounded-xl border flex items-center gap-1 ${isDark ? "bg-slate-950 border-slate-800" : "bg-slate-100 border-slate-200"}`}
          >
            <button
              type="button"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div className="flex items-center justify-between gap-2 mb-2">
            <span
              className={`text-[10px] font-black uppercase tracking-wider ${isDark ? "text-indigo-400" : "text-indigo-600"}`}
            >
          ...
          <p
            className={`text-xs mt-1.5 leading-relaxed ${isDark ? "text-slate-400" : "text-slate-600"}`}
          >
            <span className="font-semibold text-indigo-400">
              Best Strategy:
            </span>{" "}
            Launch flash sales, push banners & WhatsApp campaign broadcasts
            during <strong>{recommendedPromo}</strong>.
          </p>
        </div>
        ...
        >
          ...
          <p
            className={`text-xs mt-1.5 leading-relaxed ${isDark ? "text-slate-400" : "text-slate-600"}`}
          >
            Peak window recorded{" "}
            <strong>{heatmapData?.peakHourCount || 0} interactions/hr</strong>.
            Server memory cache & queries respond in &lt;15ms.
          </p>
        </div>
        ...
        <div
          className={`p-4 rounded-2xl border transition-all ${
            isDark
              ? "bg-slate-950/60 border-slate-800/80 hover:border-blue-500/40"
              : "bg-slate-50/70 border-slate-200 hover:border-blue-300"
          }`}
        >
          <div className="flex items-center justify-between gap-2 mb-2">
            <span
              className={`text-[10px] font-black uppercase tracking-wider ${isDark ? "text-blue-400" : "text-blue-600"}`}
            >
          ...
          <p
            className={`text-xs mt-1.5 leading-relaxed ${isDark ? "text-slate-400" : "text-slate-600"}`}
          >
            Ideal off-peak timeframe for bulk inventory CSV uploads, catalog
            price re-indexing, and store resets with zero customer disruption.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <span
            className={`text-xs font-black uppercase tracking-wider flex items-center gap-1.5 ${isDark ? "text-slate-400" : "text-slate-600"}`}
          >
          ...
          <div className="flex items-center gap-2 text-[11px] font-medium">
            ...
            <div className="flex items-center gap-1">
              <div
                className={`w-3.5 h-3.5 rounded-sm border ${isDark ? "bg-slate-900/60 border-slate-800" : "bg-slate-100 border-slate-200"}`}
              ></div>
        ...
        <div className="overflow-x-auto pb-2 pt-1">
          <div className="min-w-[760px] space-y-1">
            ...
            <div className="grid grid-cols-[56px_repeat(24,1fr)] gap-1 text-center">
              <div className="text-[10px] font-bold text-slate-500 uppercase text-left pl-1">
                Day
              </div>
              {HOURS_24.map((h) => (
                <div
                    ...
                    <button
                      ...
                      className={`h-7 rounded-lg border transition-all duration-150 flex items-center justify-center text-[9px] font-mono cursor-pointer relative group ${intensityClass} ${
                        isHovered || isSelected
                          ? "scale-110 z-20 ring-2 ring-indigo-400 shadow-lg"
                          : ""
                      }`}
      ...
      {activeInspectCell && (
        <div
          className={`p-4 rounded-2xl border transition-all animate-in fade-in duration-150 ${
            isDark
              ? "bg-slate-950 border-indigo-500/30 shadow-xl shadow-indigo-950/30"
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-3 mb-3 border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                >
                  {activeInspectCell.dayFullName}, {activeInspectCell.hourLabel}{" "}
                  –{" "}
                  {(activeInspectCell.hour + 1) % 24 === 0
                    ? 12
                    : (activeInspectCell.hour + 1) % 24 > 12
                      ? ((activeInspectCell.hour + 1) % 24) - 12
                      : (activeInspectCell.hour + 1) % 24}{" "}
                  {activeInspectCell.hour + 1 >= 12 &&
                  activeInspectCell.hour + 1 < 24
                    ? "PM"
                    : "AM"}{" "}
                  (EAT)
                ...
                >
                  {activeInspectCell.count > 0
                    ? `${activeInspectCell.uniqueVisitors} unique customer session(s) active during this hour`
                    : "No interactions recorded in this time slot"}
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5 text-center">
            <div
              className={`p-2.5 rounded-xl border ${isDark ? "bg-slate-900/60 border-slate-800" : "bg-white border-slate-200"}`}
            >
              <span
                className={`text-[10px] font-black uppercase tracking-wider block ${isDark ? "text-slate-400" : "text-slate-500"}`}
              >
                Total Actions
              </span>
              <span
                className={`text-base font-black mt-0.5 block ${isDark ? "text-white" : "text-slate-900"}`}
              >
            ...
            >
              <span
                className={`text-[10px] font-black uppercase tracking-wider block ${isDark ? "text-blue-400" : "text-blue-600"}`}
              >
                Product Views
              </span>
            ...
            >
              <span
                className={`text-[10px] font-black uppercase tracking-wider block ${isDark ? "text-amber-400" : "text-amber-600"}`}
              >
                Store Searches
              </span>
            ...
            >
              <span
                className={`text-[10px] font-black uppercase tracking-wider block ${isDark ? "text-indigo-400" : "text-indigo-600"}`}
              >
                Cart Adds
              </span>
            ...
            >
              <span
                className={`text-[10px] font-black uppercase tracking-wider block ${isDark ? "text-emerald-400" : "text-emerald-600"}`}
              >
                Orders Placed
              </span>
      ...
      {heatmapData?.hourlyDistribution &&
        heatmapData.hourlyDistribution.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pt-2 border-t border-slate-200 dark:border-slate-800">
            ...
            <div className="lg:col-span-8 space-y-2">
              <div className="flex items-center justify-between">
                ...
                >
                  Total {heatmapData.totalHeatmapInteractions} interactions
                  mapped
                </span>
              </div>
              ...
              <div className="grid grid-cols-24 gap-1 h-20 items-end pt-4 pb-1">
                {heatmapData.hourlyDistribution.map((hItem) => {
                  const maxH = Math.max(
                    ...heatmapData.hourlyDistribution.map((x) => x.count),
                    1,
                  );
                  const heightPct = Math.max(
                    Math.round((hItem.count / maxH) * 100),
                    4,
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[8px] font-mono text-slate-500 hidden sm:block truncate">
                        {hItem.hour % 4 === 0
                          ? formatHourShort(hItem.hour)
                          : ""}
                      </span>

                      {/* Tooltip on hover */}
                      <div
                        className={`absolute bottom-full mb-1 hidden group-hover:flex flex-col items-center px-2 py-1 rounded-lg text-[10px] font-bold shadow-xl border z-30 pointer-events-none whitespace-nowrap ${
                          isDark
                      ...
                      >
                        <span>
                          {hItem.hourLabel}: {hItem.count} actions
                        </span>
                      </div>
                    </div>
                  );
            ...
            <div className="lg:col-span-4 space-y-2">
              ...
              <div className="space-y-1.5">
                {heatmapData.dailyDistribution?.map((dItem) => {
                  const maxD = Math.max(
                    ...heatmapData.dailyDistribution.map((x) => x.count),
                    1,
                  );
                  const pct = Math.max(
                    Math.round((dItem.count / maxD) * 100),
                    2,
                  );
                  const isTopDay =
                    dItem.dayIndex === heatmapData.busiestDayIndex;

                  return (
                    <div
                      key={`d-rank-${dItem.dayIndex}`}
                      className="space-y-0.5"
                    >
                      <div className="flex justify-between text-xs">
                        <span
                          className={`font-bold ${isTopDay ? "text-indigo-400" : isDark ? "text-slate-300" : "text-slate-700"}`}
                        >
                      ...
                      <div
                        className={`h-1.5 w-full rounded-full overflow-hidden ${isDark ? "bg-slate-800" : "bg-slate-200"}`}
                      >
                        <div
                          style={{ width: `${pct}%` }}
