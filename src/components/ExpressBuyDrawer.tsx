...
export const ExpressBuyDrawer: React.FC<ExpressBuyDrawerProps> = ({
  ...
  useEffect(() => {
    if (user || profile) {
      setCustomerName(
        profile?.fullName || profile?.full_name || user?.displayName || "",
      );
  ...
  const unitPrice = (() => {
    ...
    const costPrice = Number(
      (product as any).cost_price || product.costPrice || 0,
    );
  ...
  const isDarEsSalaam =
    (deliveryCity || "").trim().toLowerCase() === "dar es salaam";
  ...
  const handlePlaceExpressOrder = async (e: React.FormEvent) => {
    ...
    if (!customerPhone.trim()) {
      setErrorMessage(
        "Please enter your active WhatsApp or calling phone number.",
      );
      return;
    }
    ...
    try {
      ...
      const vatTax = isVatApplied
        ? Math.round(totalAmount * (storeVatPct / (100 + storeVatPct)))
        : 0;
      const netSubtotal =
        isVatApplied && vatTax > 0 ? totalAmount - vatTax : totalAmount;

      const newOrderPayload: Omit<
        Order,
        "id" | "createdAt" | "trackingNumber"
      > = {
        ...
        customerEmail:
          customerEmail.trim() ||
          `${customerPhone.replace(/[^0-9]/g, "")}@expresscustomer.tz`,
      ...
      setErrorMessage(
        err?.message || "Failed to submit express order. Please try again.",
      );
      triggerHaptic("error");
    } finally {
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-100 font-medium">
                    Fast genuine delivery across Tanzania
                  </p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 custom-scrollbar">
              {completedOrder ? (
                (() => {
                  ...
                  const defaultStoreVat = Number(
                    storeSettings?.vatPercentage ?? 18,
                  );
                  const hasOrderItems =
                    completedOrder.items && completedOrder.items.length > 0;
                  const allItemsExempt = hasOrderItems
                    ? completedOrder.items.every(
                  ...
                  const isVatApplicable =
                    ...
                    (completedOrder.vatPercentage !== undefined
                      ? Number(completedOrder.vatPercentage) > 0
                      : defaultStoreVat > 0) &&
                    (completedOrder.tax !== undefined
                      ? Number(completedOrder.tax) > 0
                      : true);
                  const effectiveVatPct = isVatApplicable
                    ? Number(completedOrder.vatPercentage ?? defaultStoreVat)
                    : 0;
                  const vatTax =
                    isVatApplicable && effectiveVatPct > 0
                      ? completedOrder.tax !== undefined
                        ? Number(completedOrder.tax)
                        ...
                        <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200 space-y-1 text-[10px]">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="text-slate-800">
                              {formatToGMT3(completedOrder.createdAt)}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-600 font-semibold">
                        ...
                        <div className="bg-amber-50 border border-amber-200 rounded-lg p-2.5 flex items-start gap-2 text-[9.5px] text-amber-900">
                          ...
                          <div>
                            ...
                            <p className="text-amber-800">
                              ...
                              official TRA Fiscal Cash Receipt will be issued
                              once our store staff verifies your payment.
                            </p>
                          </div>
                        </div>
                ...
                <form onSubmit={handlePlaceExpressOrder} className="space-y-5">
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <h4 className="font-extrabold text-xs mt-1 truncate">
                        {product.name}
                      </h4>
                      ...
                      {quantity >= 3 ? (
                        ...
                        <div className="p-2 rounded-xl bg-purple-500/10 border border-purple-500/20 text-purple-600 dark:text-purple-400 text-[11px] font-bold flex items-center justify-between">
                          <span className="flex items-center gap-1.5">
                            ...
                            <span>
                              ⚡ 2 Units Dynamic Value Discount Applied! (Set
                              Qty to 3+ for Wholesale Rate)
                            </span>
                          </span>
                        </div>
                      ) : (
                        <div className="p-2 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-[11px] font-medium flex items-center gap-1.5">
                          ...
                          <span>
                            Buy 2 units for Dynamic Discount, or 3+ units for
                            Wholesale Pricing!
                          </span>
                        </div>
                      )}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label
                          htmlFor="express-buy-customer-name"
                      ...
                      <div>
                        <label
                          htmlFor="express-buy-customer-phone"
                          className="block text-[11px] font-bold text-slate-600 dark:text-slate-300 mb-1"
                        >
                        ...
                        <input
                          ...
                          value={customerPhone}
                          onBlur={() =>
                            setCustomerPhone(formatTzPhone(customerPhone))
                          }
                      ...
                      {(() => {
                            ...
                            : [
                                {
                                  ...
                                  accountNumber: "0150 8829 4100",
                                  instructions:
                                    "Transfer directly to CRDB account. Quote order number as reference.",
                                  isActive: true,
                                },
                                {
                                  id: "2",
                                  ...
                                  accountNumber: "0768 929 203",
                                  instructions:
                                    "Pay via Till / Lipa Namba 0768 929 203.",
                                  isActive: true,
                                },
                                {
                                  id: "3",
                                  type: "Cash on Delivery",
                                  provider:
                                    "Cash on Delivery / In-Store Pickup",
                                  accountName: "Genuine Electronics Ltd",
                                  accountNumber: "Pay Upon Delivery",
                                  instructions:
                                    "Pay cash or mobile money upon receiving your order.",
                                ...
                                },
                                {
                                  ...
                                  accountNumber: "ORBI-9901",
                                  instructions:
                                    "Pay via Orbi Pay barcode or wallet ID.",
                                  isActive: false,
                                },
                              ];
                        ...
                        return configured.map((pm) => {
                          const getPaymentColor = (provider: string) => {
                            ...
                            if (p.includes("mixx") || p.includes("tigo"))
                          ...
                          const isSelected =
                            !isDisabled && paymentMethod === pm.provider;
                          ...
                          return (
                            <motion.button
                              ...
                              className={`p-2.5 rounded-xl border text-left flex flex-col justify-between transition-all relative overflow-hidden ${
                                isDisabled
                                  ? isOrbi
                                    ? isDark
                                      ? "border-purple-900/50 bg-purple-950/20 opacity-80 cursor-not-allowed"
                                      : "border-purple-200 bg-purple-50/40 opacity-80 cursor-not-allowed"
                                    : isDark
                                      ? "border-amber-900/40 bg-amber-950/20 opacity-75 cursor-not-allowed"
                                      : "border-amber-200 bg-amber-50/40 opacity-75 cursor-not-allowed"
                                  : isSelected
                            ...
                            >
                              <div className="flex items-center justify-between w-full gap-1">
                                <span
                                  className={`text-[10px] font-extrabold truncate ${getPaymentColor(pm.provider)}`}
                                >
                                  ● {pm.type}
                                </span>
                                {isOrbi ? (
                  ...
                  >
                    ...
                    <div className="flex justify-between">
                      <span className="text-slate-400">
                        {isDarEsSalaam
                          ...
                          : `Estimate Delivery Cost (${deliveryCity})`}
                      </span>
                    ...
                    {product.isVatInclusive !== false &&
                      Number(storeSettings?.vatPercentage ?? 18) > 0 && (
                        <div className="flex justify-between text-slate-400">
                          <span>
                            TRA VAT (
                            {Number(storeSettings?.vatPercentage ?? 18)}%
                            Included)
                          </span>
                  ...
                  >
                    {isSubmitting ? (
                      ...
                      <>
                        ...
                        <span>
                          Place Express Order · {formatTZS(totalAmount)}
                        </span>
