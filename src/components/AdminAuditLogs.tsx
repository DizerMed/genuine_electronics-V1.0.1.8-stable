...
} from "../lib/enterpriseAuditService";
import {
  ShieldCheck,
  Search,
  Filter,
...
export const AdminAuditLogs: React.FC<AdminAuditLogsProps> = ({
    ...
    return logs.filter((log) => {
      ...
      const matchRole =
        ...
        String(log.actorRole || "")
          .toLowerCase()
          .includes(String(selectedRole || "").toLowerCase());
  ...
  const stats = useMemo(() => {
    return {
      ...
      financial: logs.filter(
        (l) =>
          l.action.includes("SALE") ||
          l.action.includes("LOAN") ||
          l.action.includes("REPAYMENT"),
      ).length,
      security: logs.filter(
        (l) => l.action.includes("STAFF") || l.action.includes("PERMISSION"),
      ).length,
      notifications: logs.filter(
        (l) => l.action.includes("NOTIFICATION") || l.action.includes("SMS"),
      ).length,
    };
  }, [logs]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
            Immutable, tamper-evident chronological activity logs across
            inventory adjustments, POS transactions, loan repayments, and staff
            actions.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          >
            <RefreshCw
              className={`w-3.5 h-3.5 ${isLoading ? "animate-spin text-blue-500" : ""}`}
            />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        <div
          className={`p-3.5 rounded-2xl border ${isDark ? "bg-slate-900/60 border-slate-800" : "bg-white border-slate-200 shadow-sm"}`}
        >
        ...
        <div
          className={`p-3.5 rounded-2xl border ${isDark ? "bg-slate-900/60 border-slate-800" : "bg-white border-slate-200 shadow-sm"} col-span-2 sm:col-span-1`}
        >
      ...
      {/* Filter & Search Bar */}
      <div
        className={`p-3.5 rounded-2xl border space-y-3 ${isDark ? "bg-slate-900/70 border-slate-800" : "bg-white border-slate-200 shadow-sm"}`}
      >
      ...
      {/* Logs Table / List */}
      <div
        className={`rounded-2xl border overflow-hidden ${isDark ? "bg-slate-900/60 border-slate-800" : "bg-white border-slate-200 shadow-sm"}`}
      >
        {isLoading ? (
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    ...
                    <div className="flex items-start sm:items-center gap-3">
                      <div
                        className={`p-2 rounded-xl border flex items-center justify-center shrink-0 ${badge.bg}`}
                      >
                    ...
                    <div className="mt-3 pl-11">
                      ...
                      >
                        <span>
                          {isExpanded
                            ? "Hide changes diff"
                            : `Inspect field diff (${log.changesSummary.length} fields)`}
                        </span>
                        {isExpanded ? (
                      ...
                      {isExpanded && (
                        <div className="mt-2 p-3 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2 font-mono text-[11px]">
                          {log.changesSummary.map((diff, idx) => (
                            <div
                              key={idx}
                              className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-slate-300"
                            >
