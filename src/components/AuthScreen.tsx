...
export function AuthScreen({
    ...
    try {
      if (mode === "forgot_password") {
        ...
        if (resetMethod === "email" || isAdminEmail) {
          ...
          try {
            ...
            if (isSupabaseConfigured) {
              ...
              if (supaErr)
                throw new Error(
                  supaErr.message || "Failed to send reset email.",
                );
              setSuccessMsg(
                `A secure recovery link has been dispatched to ${cleanEmail}.`,
              );
            } else {
              throw emailErr;
            }
        ...
        if (!resetSuccess && isSupabaseConfigured) {
          try {
            const { data: supaUser } = await supabaseClient.auth.updateUser({
              password,
            });
            if (supaUser?.user) {
              sessionUser = {
        ...
        if (!resetSuccess) {
          throw new Error(
            serverErrorMsg ||
              "Password reset failed. Please ensure the email and details are correct.",
          );
        }

        setSuccessMsg(
          "Your password was successfully reset! You are being signed in...",
        );
        if (sessionUser) {
      ...
      if (!authenticatedUser && isSupabaseConfigured) {
        if (mode === "signup") {
          const { data: supaSignUp, error: supaErr } =
          ...
          const { data: supaSignIn, error: supaErr } =
          ...
          if (supaSignIn?.user) {
            authenticatedUser = {
              ...
              email: cleanEmail,
              displayName:
                supaSignIn.user.user_metadata?.full_name ||
                cleanEmail.split("@")[0],
              fullName: supaSignIn.user.user_metadata?.full_name,
              role: isAdminEmail
                ? "admin"
                : supaSignIn.user.user_metadata?.role || "customer",
            };
          }
        }
      }
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-sm transition-colors duration-300 ${
    ...
    >
      ...
      >
          ...
          <div className="flex flex-col items-center mb-6 text-center">
            ...
            {mode === "forgot_password" ? (
              <>
                ...
                <h2
                  className={`text-2xl font-black mb-1.5 ${isDark ? "text-white" : "text-slate-900"}`}
                >
                  Reset Password
                </h2>
                <p
                ...
                <div
                  className={`grid grid-cols-2 gap-1.5 p-1 rounded-xl mt-3 mb-1 border ${isDark ? "bg-slate-800/80 border-slate-700" : "bg-slate-100 border-slate-200"}`}
                >
                  <button
                    type="button"
                    onClick={() => {
              ...
              <>
                ...
                <p
                  className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}
                >
                  Create an account to track orders and save VIP delivery
                  addresses.
                </p>
              </>
            ) : (
              <>
                ...
                <p
                  className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}
                >
                  Sign in to access your customer account, past orders, and
                  tracking.
                </p>
              </>
            )}
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            ...
            {mode === "forgot_password" && resetMethod === "direct" && (
              <div>
                ...
                <p className="text-[10px] text-slate-500 mt-1">
                  Used to securely verify account ownership without sending an
                  email link.
                </p>
              </div>
            )}
            ...
            <div className="relative my-4">
              <div className="absolute inset-0 flex items-center">
                <div
                  className={`w-full border-t ${isDark ? "border-slate-800" : "border-slate-100"}`}
                ></div>
              </div>
              <div className="relative flex justify-center text-[10px] uppercase tracking-wider font-bold">
                <span
                  className={`px-3 ${isDark ? "bg-slate-900 text-slate-500" : "bg-white text-slate-400"}`}
                >
                  {mode === "forgot_password"
        ...
        <div
          className={`px-6 py-3 text-center border-t text-[10px] uppercase tracking-[0.2em] font-bold shrink-0 ${
            isDark
