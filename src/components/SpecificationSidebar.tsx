...
const SPEC_PATTERNS = {
  ...
  storage:
    /\b(32|64|128|256|512)\s*(?:GB|gb)\s*(?:SSD|NVMe|Storage|ROM|eMMC)?\b|\b(1|2|4)\s*(?:TB|tb)\s*(?:SSD|NVMe|HDD|Storage)?\b/i,
  screenSize:
    /\b(5\.8|6\.1|6\.2|6\.3|6\.4|6\.5|6\.6|6\.7|6\.8|6\.9|11|12\.9|13|13\.3|13\.6|14|14\.2|15|15\.6|16|16\.2|24|27|32|40|43|50|55|65|70|75|85)\s*(?:["”]|inch|Inch|-inch|in\b)/i,
  resolution:
    /\b(4K|8K|UHD|QLED|OLED|Mini-LED|Full HD|1080p|Crystal UHD|Retina|Liquid Retina|Super AMOLED|AMOLED)\b/i,
...
export const SpecificationSidebar: React.FC<SpecificationSidebarProps> = ({
  ...
  const quickPriceTiers = useMemo(() => {
    ...
    if (min < 300000 && max > 200000) {
      tiers.push({
        label: isSwahili ? "Chini ya 250K" : "Under 250K",
        min: null,
        max: 250000,
      });
    }
  ...
  const extractedSpecsData = useMemo(() => {
    ...
    products.forEach((p) => {
      ...
      if (p.isOnOffer || (p.originalPrice && p.originalPrice > p.price))
        onSaleCount++;
      ...
      if (p.energyRating && p.energyRating.trim()) {
        ...
        specsMap["Energy / Power"][val] =
          (specsMap["Energy / Power"][val] || 0) + 1;
      }
      ...
      if (p.capacity && p.capacity.trim()) {
        ...
        specsMap["Capacity / Size"][val] =
          (specsMap["Capacity / Size"][val] || 0) + 1;
      }
      ...
      if (p.specs && typeof p.specs === "object") {
        Object.entries(p.specs).forEach(([k, v]) => {
          ...
          if (lowerK.includes("ram") || lowerK.includes("memory")) {
            specsMap["RAM / Memory"][cleanV] =
              (specsMap["RAM / Memory"][cleanV] || 0) + 1;
          } else if (
            lowerK.includes("storage") ||
            lowerK.includes("rom") ||
            lowerK.includes("ssd") ||
            lowerK.includes("hdd") ||
            lowerK.includes("capacity")
          ) {
            specsMap["Storage / ROM"][cleanV] =
              (specsMap["Storage / ROM"][cleanV] || 0) + 1;
          } else if (
            lowerK.includes("screen") ||
            lowerK.includes("display size") ||
            lowerK.includes("inch")
          ) {
            specsMap["Screen Size"][cleanV] =
              (specsMap["Screen Size"][cleanV] || 0) + 1;
          } else if (
            lowerK.includes("resolution") ||
            lowerK.includes("display") ||
            lowerK.includes("panel") ||
            lowerK.includes("refresh")
          ) {
            specsMap["Display & Resolution"][cleanV] =
              (specsMap["Display & Resolution"][cleanV] || 0) + 1;
          } else if (
            lowerK.includes("processor") ||
            lowerK.includes("cpu") ||
            lowerK.includes("chip") ||
            lowerK.includes("gpu")
          ) {
            specsMap["Processor / Chip"][cleanV] =
              (specsMap["Processor / Chip"][cleanV] || 0) + 1;
          } else {
      ...
      if (ramMatch && ramMatch[1]) {
        ...
        specsMap["RAM / Memory"][val] =
          (specsMap["RAM / Memory"][val] || 0) + 1;
      }
      ...
      if (storageMatch) {
        ...
        specsMap["Storage / ROM"][val] =
          (specsMap["Storage / ROM"][val] || 0) + 1;
      }
      ...
      if (resMatch && resMatch[0]) {
        ...
        specsMap["Display & Resolution"][val] =
          (specsMap["Display & Resolution"][val] || 0) + 1;
      }
    });
    ...
    const getSpecIcon = (key: string) => {
      ...
      if (lower.includes("ram") || lower.includes("memory"))
        return <MemoryStick className="w-4 h-4 text-blue-500" />;
      if (lower.includes("storage") || lower.includes("rom"))
      ...
      if (lower.includes("display") || lower.includes("resolution"))
    ...
    return {
      brands: Object.entries(brandsMap)
        ...
        .sort((a, b) => b.count - a.count),
      warranties: Object.entries(warrantiesMap)
        .map(([warranty, count]) => ({ warranty, count }))
        .sort((a, b) => b.count - a.count),
  ...
  const activeFiltersCount = useMemo(() => {
    ...
    if (filters.selectedWarranties.length > 0)
      count += filters.selectedWarranties.length;
  ...
  const handleApplyPriceInputs = () => {
    ...
    onFilterChange({
      ...filters,
      minPrice:
        !isNaN(Number(minVal)) && minVal !== null ? Math.max(0, minVal) : null,
      maxPrice:
        !isNaN(Number(maxVal)) && maxVal !== null ? Math.max(0, maxVal) : null,
    });
  };
  ...
  const displayedBrands = useMemo(() => {
    ...
    return extractedSpecsData.brands.filter((b) =>
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
              ...
              {isSwahili ? "vifaa vipo" : "matching products"}
            </span>
          </div>
        </div>
          ...
          <div className="flex items-center gap-2.5">
            <input
              ...
              onChange={() =>
                onFilterChange({
                  ...filters,
                  genuineOnly: !filters.genuineOnly,
                })
              }
              className="w-4 h-4 rounded text-blue-600 focus:ring-blue-500 cursor-pointer"
            />
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between px-1">
            <span className="text-[11px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500 flex items-center gap-1">
              ...
              <span>
                {isSwahili ? "Sifa za Kiufundi" : "Technical Specifications"}
              </span>
            </span>
            {Object.keys(filters.selectedSpecs).length > 0 && (
              <button
                type="button"
                onClick={() =>
                  onFilterChange({ ...filters, selectedSpecs: {} })
                }
                      ...
                      {options.map(({ value, count }) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span>{value}</span>
                            <span
                              className={`text-[10px] ${isSelected ? "text-blue-100" : "opacity-60"}`}
                            >
                              ({count})
                            </span>
                          </button>
          ...
          {openSections.warranty && (
            <div className="p-3 border-t border-slate-100 dark:border-slate-800/80 space-y-1">
              {extractedSpecsData.warranties.map(({ warranty, count }) => {
                const isSelected =
                  filters.selectedWarranties.includes(warranty);
                return (
                  <label
                    key={warranty}
                    className={`flex items-center justify-between p-1.5 rounded-xl transition-colors cursor-pointer text-xs ${
  ...
  return (
    <>
      ...
      >
        <div
          className={`sticky top-24 max-h-[calc(100vh-7rem)] flex flex-col p-4 rounded-3xl border shadow-xs transition-all ${
            isDark
            ...
            >
              ...
              >
                ...
                >
                  <span>
                    {isSwahili
                      ? `Onyesha Vifaa ${totalMatchingCount}`
                      : `Show ${totalMatchingCount} Products`}
                  </span>
                </button>
              </div>
            </motion.div>
  ...
  filters.selectedWarranties.forEach((warranty) => {
    chips.push({
      ...
      onRemove: () =>
        onFilterChange({
          ...filters,
          selectedWarranties: filters.selectedWarranties.filter(
            (w) => w !== warranty,
          ),
        }),
    });
