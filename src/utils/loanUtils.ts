 ...
 */
export function getLoanCustomerName(
  tx: (Partial<POSTransaction> & Record<string, any>) | null | undefined,
): string {
  ...
  if (tx.customer) {
    if (
      ...
      tx.customer.trim() &&
      tx.customer.trim().toLowerCase() !== "walk-in customer" &&
      tx.customer.trim().toLowerCase() !== "walk-in"
    ) {
    ...
    if (typeof tx.customer === "object") {
      ...
      if (
        ...
        typeof cName === "string" &&
        cName.trim() &&
        cName.trim().toLowerCase() !== "walk-in customer"
      ) {
        return cName.trim();
      }
    }
  ...
  if (tx.shippingAddress) {
    if (typeof tx.shippingAddress === "object") {
      const sName =
        (tx.shippingAddress as any).fullName ||
        (tx.shippingAddress as any).name ||
        (tx.shippingAddress as any).customerName;
      if (sName && typeof sName === "string" && sName.trim()) {
        return sName.trim();
      }
    } else if (
      typeof tx.shippingAddress === "string" &&
      tx.shippingAddress.trim() &&
      !tx.shippingAddress.startsWith("{")
    ) {
  ...
  if (
    ...
    typeof tx.loanGuarantorName === "string" &&
    tx.loanGuarantorName.trim()
  ) {
 ...
 */
export function getLoanCustomerPhone(
  tx: (Partial<POSTransaction> & Record<string, any>) | null | undefined,
): string {
  if (!tx) return "";
  const phone =
    ...
    tx.customer_phone ||
    tx.customerphone ||
    (typeof tx.customer === "object"
      ? (tx.customer as any)?.phone || (tx.customer as any)?.telephone
      : "") ||
 ...
 */
export function getLoanDueDate(
  tx: (Partial<POSTransaction> & Record<string, any>) | null | undefined,
): string {
  if (!tx) return "";
  ...
  const rawStr =
    typeof dateVal === "string" ? dateVal.trim() : String(dateVal || "").trim();
  if (!rawStr) return "";
 ...
 */
export function getLoanNationalId(
  tx: (Partial<POSTransaction> & Record<string, any>) | null | undefined,
): string {
  if (!tx) return "";
  if (tx.isLoan === false || tx.is_loan === false) return "";
...
export function getLoanGuarantor(
  ...
  if (tx.isLoan === false || tx.is_loan === false)
    return { name: "", phone: "" };
  const name = String(
    tx.loanGuarantorName ||
...
export function isLoanTransaction(
  ...
  const loanBal = Number(
    tx.loanBalance ??
      tx.loan_balance ??
      tx.outstandingBalance ??
      tx.outstanding_balance ??
      0,
  );
 ...
 */
export function computeLoanMeta(
  tx: (Partial<POSTransaction> & Record<string, any>) | null | undefined,
): LoanMeta {
  if (!tx) {
    return {
  ...
  const repaymentsList: any[] =
    tx.loanRepayments ||
    tx.loan_repayments ||
    tx.partialPayments ||
    tx.partial_payments ||
    [];
  const repaymentsSum = repaymentsList.reduce(
