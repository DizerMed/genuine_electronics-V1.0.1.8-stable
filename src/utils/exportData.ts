...
export function exportLoansToCSV(loans: POSTransaction[]) {
  ...
  const rows = loans.map((l) => {
    ...
    const repaymentsSum = (l.loanRepayments || []).reduce(
 ...
 */
export function exportTaxJournalToCSV(
  transactions: POSTransaction[],
  vatRatePercentage: number = 18,
) {
  const headers = [
    "Date (EAT)",
    "Receipt Number",
  ...
  const rows = transactions.map((t) => {
    ...
    (t.items || []).forEach((item) => {
      if (item.serialNumbers && item.serialNumbers.length > 0) {
        allSerials.push(
          `${item.product.name}: [${item.serialNumbers.join(", ")}]`,
        );
      }
    });
    ...
    const priceTierSummary =
      t.priceTier ||
      ((t.items || []).some((i) => i.priceTier === "wholesale")
        ? "Wholesale"
        : "Retail");
