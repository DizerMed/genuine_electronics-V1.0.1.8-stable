...
export const useLoanAlerts = (
  ...
  useEffect(() => {
    ...
    orders.forEach((order) => {
      ...
      const orderUserId =
        order.userId || order.user_id || order.customerId || order.customer_id;
      if (
        orderUserId === userId &&
        ...
        const isOverdueMilestone = [-1, -3, -7, -15, -30, -60].includes(
          diffDays,
        );
        ...
        if (isUpcoming || isOverdueMilestone) {
          const alertItem = {
            ...
            orderId: order.id,
            productSummary:
              order.items.map((i) => i.product.name).join(", ") || "Loan",
          ...
          return (
            <div
              ...
              className={`pointer-events-auto overflow-hidden rounded-2xl border ${
                isOverdue
                  ? "bg-rose-50 border-rose-200 dark:bg-rose-950/40 dark:border-rose-900/50"
                  : isToday
                ...
                <div className="flex-1 pr-4">
                  ...
                  <p className="text-xs font-medium text-slate-600 dark:text-slate-300 mt-1">
                    ...
                    <span className="font-bold text-slate-900 dark:text-white">
                      {formatTZS(alert.remainingBalance)}
                    </span>{" "}
                    for{" "}
                    <span className="font-bold text-slate-800 dark:text-slate-200">
                      {alert.productSummary}
                    </span>
                    .
                  </p>
                  <div className="mt-2.5 flex items-center justify-between">
                    ...
                    >
                      {isOverdue
                        ? `${Math.abs(alert.daysRemaining)} Days Late`
                        : isToday
                          ? "Due Today"
